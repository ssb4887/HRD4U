...
$('.btn-back').on('click', function() {
	...
	if(previousUrl.includes('list.do')) {
		window.location.href = previousUrl;
	} else if(previousUrl.includes('diagnosis')) {
		window.location.href = contextPath + '/dct/diagnosis/list.do?mId=36';
	} else {
		window.location.href = contextPath + '/dct/bsisCnsl/list.do?mId=37';
	}
});
...
$.ajax({
	...
	url : `${contextPath}/dct/bsisCnsl/qustnrData.do?mId=37`,
...
const saveQustnrData = (flag) => {
	...
	fetch(`${contextPath}/dct/bsisCnsl/insertQustnr.do?mId=37`, {
		method: "POST",
		body: sortedFormData
	})
	.then((response) => {
	...
	.then((data) => {
		...
		document.getElementById('rslt').value = data.param.rsltIdx;
		document.getElementById('bsc').value = data.param.bscIdx;
