...
viewButtons.forEach(function(viewBtn) {
	viewBtn.addEventListener('click', function(event) {
		...
		const actionUrl = `${contextPath}/`+siteId+`/supportForm/view.do?mId=115`;
...
inputButtons.forEach(function(inputBtn) {
	inputBtn.addEventListener('click', function(event) {
		...
		const form = document.getElementById('form-box');
		const actionUrl = `${contextPath}/`+siteId+`/supportForm/input.do?mId=115`;
		form.action = actionUrl;
		document.getElementById('idx').value = idx;
		console.log(idx);
		form.submit();
...
if(excelBtn) {
	excelBtn.addEventListener('click', function() {
		...
		const actionUrl = `${contextPath}/dct/supportForm/excel.do` + queryString;
		form.action = actionUrl;
		form.submit();
	});
}