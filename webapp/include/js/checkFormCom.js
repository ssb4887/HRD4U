...
/******************************************
숫자만 입력 : 붙여넣기할 때 숫자를 제외하고 모두제거
******************************************/
function fn_onlyNumNoCopy(event){
	const regExp = /[^0-9]/g;
...
/******************************************
숫자만 입력 : 금액을 입력할 때 사용(자리수 구분이 필요할 때)
******************************************/
...
/******************************************
자리수 표현 : fn_onlyNumForCost와 같이 사용
******************************************/
function fn_costRegExp(event) {
	const regExp = /[0-9]/g;
	const ele = event.target;
	if(regExp.test(ele.value)){
		let value = ele.value;
...
/******************************************
숫자('-' 포함)만 입력 : 전화번호 입력할 때 사용, 붙여넣기할 때 숫자, '-'를 제외하고 모두제거
******************************************/
function fn_onlyNumDashNoCopy(event){
	const regExp = /[^-0-9]/g;
	const ele = event.target;
	if(regExp.test(ele.value)){
		ele.value = ele.value.replace(regExp, '');
...
var fn_dialog_div = {
		...
		, init:function(theDId, theZIndex, theWidth, theHeight){
			...
			var varIsIfm = $("#" + fn_dialog_div.fId).length > 0;
...
function fn_isFill(theInput) {
	try{
		var varObj = $(theInput);
		if(varObj.length == 0) return true;
...
function fn_checkFileSelectOption(theInputLayer, theText, theGtIdx, theMinSize, theMaxSize){
	try{
		...
		if(varObjLayer.length == 0) return true;
...
function fn_checkElementChecked(theName, theText){
	try{
		...
		var varChkObjSize = varChkObjs.length;
		if(varChkObjSize == 0) {
			alert(fn_Message.checkText(theText));
...
function fn_validEmail(theObj) {
	if (typeof theObj == "undefined" || $(theObj).length == 0) {
		return true;
	}
...
function fn_moveUDOptions(theObj, theFlag, theSIdx) {
	...
	var varSelectedCnt = $(varSelectedObj).length;
	...
	if(theFlag == '1') {
		...
		var varOptionMaxIdx = $(theObj).find('option').length - 1;
		$(varSelectedObj).each(function(index, item){
			if(varIsChecked) return;
			var varOptions = $(theObj).find('option');
...
function fn_moveUDLiChecks(theTxt, theObj, theFlag, theSIdx) {
	...
	if(theFlag == '1') {
		...
		// 아래
		varSelectedObj = $(varSelectedObj).get().reverse();
		var varOptionMaxIdx = $(theObj).find('>li').length - 1;
		$(varSelectedObj).each(function(index, item){
			if(varIsChecked) return;
			var varOptions = $(theObj).find('>li');
			//alert($(theObj).find('option').index(item));
