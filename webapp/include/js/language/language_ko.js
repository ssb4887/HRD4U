var fn_Message = {
	...
	fillAllText: function(theText) {
		return theText + '을/를 모두 입력해 주세요.';
	}, 
	fillText: function(theText) {
		return theText + '을/를 입력해 주세요.';
	}, 
	checkText: function(theText) {
		return theText + '을/를 선택해 주세요..';
	},
	numberText: function(theText) {
		return theText + '은/는 \'0\'으로 시작하지 않는 숫자를 입력해 주세요.';
	}, 
	itemTypeText: function (theItemTypeText, theText) {
		return theText + '은/는 ' + theItemTypeText + '만 입력할 수 있습니다.';
	},
	impossibleBeginText: function (theItemTypeText, theText, theIsText) {
		var varStr;
		if(theIsText) varStr = '\'';
		else varStr = '';
		return theText + '은/는  ' + varStr + theItemTypeText + varStr + '로 시작하는 문자는 입력할 수 없습니다.';
	},
	impossibleEndText: function (theItemTypeText, theText, theIsText) {
		var varStr;
		if(theIsText) varStr = '\'';
		else varStr = '';
		return theText + '은/는  ' + varStr + theItemTypeText + varStr + '로 끝나는 문자는 입력할 수 없습니다.';
	},
	impossibleText: function (theItemTypeText, theText, theIsText) {
		var varStr;
		if(theIsText) varStr = '\'';
		else varStr = '';
		return theText + '은/는  ' + varStr + theItemTypeText + varStr + ' 문자는 입력할 수 없습니다.';
	},
	possibleText: function (theItemTypeText, theText) {
		return theText + '은/는  \'' + theItemTypeText + '\' 문자만 입력할 수 있습니다.';
	},
	numbertextText: function(theText) {
		return theText + '은/는 숫자를 입력해 주세요';
	}, 
	limitLenText: function(theText, theMaxLen) {
		var varText = '';
		if(theText) varText = theText + '은/는 ';
		varText += "총 영문 " + (theMaxLen * 2) + "자 한글 " + theMaxLen + "자 까지만 입력할 수 있습니다.";
		return varText;
	},
	limitLenText2: function(theText, theEnMaxLen, theKoMaxLen) {
		var varText = '';
		if(theText) varText = theText + '은/는 ';
		varText += "총 영문 " + theEnMaxLen + "자 한글 " + theKoMaxLen + "자 까지만 입력할 수 있습니다.";
		return varText;
	},
	limitTLenText: function(theText, theMaxLen) {
		var varText = '';
		if(theText) varText = theText + '은/는 ';
		varText += "총  " + theMaxLen + "자 까지만 입력할 수 있습니다.";
		return varText;
	},
	limitBTLenText: function(theText, theMinLen, theMaxLen) {
		var varText = '';
		if(theText) varText = theText + '은/는 ';
		varText += theMinLen + "자이상 " + theMaxLen + "자 이내로 입력할 수 있습니다.";
		return varText;
	},
	...
	dataSelectText: function(theText) {
		return theText + '할 데이터를 1개 이상 선택해야 합니다.';
	},
	...
	validNoFirstCharText: function(theText) {
		return theText + '의 첫문자는 특수문자로 시작할 수 없습니다.';
	},
	...
	validNoPWCharText: function(theText, theMinLen, theMaxLen) {
		return theText + '는 ' + theMinLen + '자이상 ' + theMaxLen + '자 이내 영문자,숫자,특수문자를 혼용해야 합니다.';
	}, 
	validNoPWCharText2: function(theText) {
		return theText + '는 영문자,숫자,특수문자를 혼용해야 합니다.';
	}, 
	validNoPWCharText3: function(theText) {
		return theText + '는 같은 문자를 4번 이상 사용할 수 없습니다.';
	},
	validNoPWCharText4: function(theText, theText2) {
		return theText + '는 ' + theText2 + '를 포함하여 사용할 수 없습니다.';
	}, 
	...
	noUpfileText: function(theText) {
		...
		return varText + '.jsp, .java, .class, .jar, .properties, .tld, .xml, .exe, .php, .asp, .aspx, .cgi, .tag, json 형식의 파일은 업로드할 수 없습니다';
	}, 
	checkFileMaxLimitCountText: function(theText, theMaxCount) {
		return theText + '은/는 ' + theMaxCount + '이하 업로드 해야 합니다.';
	},
	checkFileMinMaxLimitCountText: function(theText, theMinCount, theMaxCount) {
		return theText + '은/는 ' + theMinCount + '개 이상 ' + theMaxCount + '이하 업로드 해야 합니다.';
	},
	uploadFileLimitCountText : function(theFileLimitCount) {
		return '업로드하실 파일의 갯수는 ' + theFileLimitCount + '개를 넘을 수 없습니다.';
	}, 
	uploadDuplicateText : '파일명이 같은 파일을 여러개 등록 할 수 없습니다.', 
	...
	menuMoveInMain:'메뉴는 메인 하위메뉴로만 이동할 수 있습니다.', 
	loginConfirmText:'로그인하셔야 사용할 수 있습니다. \n로그인하시겠습니까?', 
