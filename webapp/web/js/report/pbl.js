			...
			if(poolValidity()) {
				...
				.then((data) => {
					if(data.result.body === 'fail') {
						...
						json_obj = obj_;
						const { plagiarismTotal, plagiarismRate, plagiarismSimilar, plagiarismMatched, plagiarismSource, plagiarismTotals } = json_obj;
						const plagiarism = {'total': plagiarismTotal[0], 'rate': plagiarismRate[0], 'similar': plagiarismSimilar[0], 'source': plagiarismSource[0], 'matched': plagiarismMatched, 'totals': plagiarismTotals}
				...
				const profileTarget_e = document.querySelector('textarea#profile-target');
				const trs_ = document.querySelectorAll('tr.profile-detail');
				const profileDetail_es = Array.from(trs_).map(e => (e.querySelector('textarea'))).filter(e => !e.id.startsWith('profile-detail-feedback'))
				for(let i=0; i<profileDetail_es.length; i++) {
					const {value} = profileDetail_es[i];
					if(value === '' || value == null || value == undefined) {
						profileDetail_es[i].focus();
					}
				}
			}
		}
