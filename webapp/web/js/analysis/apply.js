...
if(parameters.get('rsltAnalsIdx') == null) {
	url = `${contextPath}`+"/web/analysis/read.do?mId=58";
} else {
	let idx = parameters.get('rsltAnalsIdx');
	url = `${contextPath}`+"/web/analysis/read.do?mId=58&rsltAnalsIdx="+idx+"&mode=m";
	let chk = 0;
	applyChk(0);
}
...
function srchGo(id){
	...
	$.ajax({
		type : "GET",
		url : `${contextPath}/web/analysis/subjectFind.do?mId=58&bplNo=`+id,
...
const delOneAnalysis = (id) => {
	...
	fetch(`${contextPath}/web/analysis/deleteData.do?mId=58`, {
		method: 'POST',
		headers: {
	...
	.then((data) => {
		if(data.result.code == 1) {
			alert("정상적으로 삭제되었습니다.")
			window.location.href='/web/analysis/list.do?mId=58';
		}
	});
}
...
const saveAnalysisData = (flag) => {
	...
	if(mode == null) {
		url = `${contextPath}`+"/web/analysis/inputProc.do?mId=58";
	} else {
		url = `${contextPath}`+"/web/analysis/inputProc.do?mId=58&rsltAnalsIdx="+idx+"&mode=m";
	}
	
	if(flag == 1) {
		if($("#id_trPurps").val() == "") {
			alert("평가목적의 작성이 완료되지 않았습니다.");
			return false;
		}
		const chk = $("input:radio[name='trResultReprtSe']:checked").val();
		if(chk == '1') {
			if($("input:radio[name='edcCrse1']:checked").length < 1) {
				alert("운영 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='edcCrse2']:checked").length < 1) {
				alert("운영 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='edcCrse3']:checked").length < 1) {
				alert("운영 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='edcCrse4']:checked").length < 1) {
				alert("환경 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='edcCrse5']:checked").length < 1) {
				alert("훈련내용 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='gnrlz']:checked").length < 1) {
				alert("강사 파트에 설문이 완료되지 않았습니다.");
				return false;
			}		
			if($("input:radio[name='knwldgTchnlgyPickup1']:checked").length < 1) {
				alert("지식·기술습득 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='knwldgTchnlgyPickup2']:checked").length < 1) {
				alert("지식·기술습득 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='knwldgTchnlgyPickup3']:checked").length < 1) {
				alert("지식·기술습득 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='knwldgTchnlgyPickup4']:checked").length < 1) {
				alert("태도변화 파트에 설문이 완료되지 않았습니다.");
				return false;
			}		
		} else if(chk == 2){
			if($("input:radio[name='stoprtApplc1']:checked").length < 1) {
				alert("훈련전이 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='stoprtApplc2']:checked").length < 1) {
				alert("훈련전이 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='stoprtApplc3']:checked").length < 1) {
				alert("훈련전이 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
			if($("input:radio[name='stoprtApplc4']:checked").length < 1) {
				alert("조직전이 파트에 설문이 완료되지 않았습니다.");
				return false;
			}
		} else {
			alert("평가항목을 선택하여주시기 바랍니다.");
			return false;
		}
		
		var result = confirm("제출하시면 수정이 불가능합니다 제출하시겠습니까?");
		if (result) {
			f.action = url;
			f.submit();
		} else {
