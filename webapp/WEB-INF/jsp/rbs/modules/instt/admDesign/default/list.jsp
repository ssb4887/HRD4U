...
<c:set var="searchFormId" value="fn_insttSearchForm"/>
<c:set var="listFormId" value="fn_insttListForm"/>
		...
		<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 수정 링크를 통해서 수정페이지로 이동합니다.">
			...
			<tbody class="alignC">
				...
				<tr>
					...
					<td><itui:objectView itemId="locCode" itemInfo="${itemInfo}" objDt="${listDt}"/></td>
					<td class="tlt"><a href="<c:out value="${URL_VIEW}"/>&${listIdxName}=${listKey}" class="fn_btn_view"><itui:objectView itemId="insttName" itemInfo="${itemInfo}" objDt="${listDt}"/></a></td>
					<td><itui:objectViewMultiCheckbox itemId="blockCode" itemInfo="${itemInfo}" objDt="${listDt}"/></td>
				</tr>
