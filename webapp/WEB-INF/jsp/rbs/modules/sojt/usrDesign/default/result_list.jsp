...
<div class="contents-area02">
	<form id="main" action="${contextPath}/${siteId}/sojt/resultList.do?mId=151" method="GET">
	...
	<table class="tbListA" summary="<c:out value="${settingInfo.list_title}"/> 목록을 볼 수 있고 제목 링크를 통해서 게시물 상세 내용으로 이동합니다.">
		...
		<tbody class="alignC" id="main">
			...
			<tr class="real-item">
				...
				<td class="">${confirmProgress.get(listDt.CONFM_STATUS) }</td>
				<%-- <td>
					<a href="${contextPath}/dct/sojt/resultForm.do?mId=151&id=${listDt.SOJT_IDX }" class="redgrad" style="text-decoration: underline;">상세보기</a>
				</td> --%>
				<c:choose>
					...
					<c:otherwise>
						<td>
							<a href="${contextPath}/dct/sojt/resultForm.do?mId=151&id=${listDt.SOJT_IDX }&bplNo=${listDt.BPL_NO}" class="redgrad" style="text-decoration: underline;">상세보기</a>
						</td>
					</c:otherwise>
				</c:choose>
			</tr>
...
async function confirm(idx) {
	...
	let result = await fetch(`${contextPath}/dct/sojt/accept.do?mId=136`,{
		method: 'POST',
		body: form_data
	})
