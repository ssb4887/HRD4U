...
<!-- CMS 시작 -->
	...
	<div class="tabmenu-wrapper01 depth6">
		<ul>
			...
			<li>
				<a href="${contextPath}/${siteId }/taskhub/program/sojt.do?mId=48" class="active">체계적현장훈련(S-OJT)</a>
			</li>
			<li>
	...
	<div class="contents-area02">
		<form action="${contextPath }/${siteId}/taskhub/program/sojt.do?mId=48" method="GET" id="main">
	...
	<div class="contents-area">
		<div class="title-wrapper">
			...
			<div class="fr">
		    	<a href="${contextPath}/${siteId}/taskhub/program/sojtExcel.do?mId=${param.mId}
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td>D+${item.PASSED }</td>
						<td>
							<a href="${contextPath }/${siteId }/sojt/applyList.do?mId=${siteId eq 'web' ? '121' : '136' }">${empty item.APPLY ? '-' : item.APPLY}</a>
							<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
							<c:if test="${(item.NOW == 'apply') and (item.PASSED >= 5)}">
							<p>
						...
						<td>
							<a href="${contextPath }/${siteId }/sojt/acceptList.do?mId=${siteId eq 'web' ? '110' : '151' }">${empty item.ACCEPT ? '-' : item.ACCEPT}</a>
							<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
							<c:if test="${(item.NOW == 'accept') and (item.PASSED >= 5)}">
							<p>
								<c:forEach begin="1" end="${passed }">
									<span class="fire"></span>
								</c:forEach>
						...
						<td>
							<a href="${contextPath }/${siteId }/sojt/acceptList.do?mId=${siteId eq 'web' ? '110' : '151'}">${empty item.PRE_APPROVAL ? '-' : item.PRE_APPROVAL}</a>
							<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
							<c:if test="${(item.NOW == 'pre_approval') and (item.PASSED >= 5)}">
							<p>
								<c:forEach begin="1" end="${passed }">
									<span class="fire"></span>
								</c:forEach>
						...
						<td>
							<a href="${contextPath }/${siteId }/sojt/acceptList.do?mId=${siteId eq 'web' ? '110' : '151' }">${empty item.FINAL_APPROVAL ? '-' : item.FINAL_APPROVAL}</a>
							<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
							<c:if test="${(item.NOW == 'final_approval') and (item.PASSED >= 5)}">
							<p>
								<c:forEach begin="1" end="${passed }">
									<span class="fire"></span>
								</c:forEach>
