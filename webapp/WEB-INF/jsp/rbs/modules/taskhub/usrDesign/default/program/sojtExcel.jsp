...
String userAgent = request.getHeader("User-Agent");
String fileName = "종합관리_SOJT선정_목록.xls";
...
<body topmargin="0" leftmargin="0" marginheight="0" marginwidth="0">
	<table style="width:100%;">
		<caption>종합관리 SOJT선정 목록</caption>
		<colgroup>
			<col width="500px" />
			<col width="500px" />
			<col width="500px" />
		...
		<tbody>
			...
			<tr>
				...
				<td>D+${item.PASSED }</td>
				<td>
					<c:out value="${empty item.APPLY ? '-' : item.APPLY}" />
				...
				<td>
					<c:out value="${empty item.ACCEPT ? '-' : item.ACCEPT}" />
					<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
				...
				<td>
					<c:out value="${empty item.PRE_APPROVAL ? '-' : item.PRE_APPROVAL}" />
					<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
				...
				<td>
					<c:out value="${empty item.FINAL_APPROVAL ? '-' : item.FINAL_APPROVAL}" />
					<c:set var="passed" value="${item.PASSED >= 10 ? 3 : item.PASSED >= 7 ? 2 : item.PASSED >= 5 ? 1 : 0 }" />
