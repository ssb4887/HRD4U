...
<!-- CMS 시작 -->
	<jsp:include page="../header/corpo.jsp" flush="false">
		...
		<jsp:param name="cPath" value="agreement" />
	</jsp:include>
	<div id="overlay"></div>
	...
	<div id="cms_calendar_article"> 	
		<div class="title-wrapper">
			...
			<div class="fr btns-area">
				...
				<c:if test="${loginVO.usertypeIdx > 55 }">
					<button class="btn-m01 btn-color01 depth3">엑셀 다운로드</button>
				</c:if>
				</div>
			</div>
		</div>
	...
	window.onload = () => {
		...
		$('tbody#main tr:has(td a#link)').each(function() {
		    ...
		    if(newUrl.includes('/taskhub')) {
		    	newUrl = newUrl.split('/taskhub')[0] + '/agreement/register.do?mId=52';
		    }
	...
	function request(prvtcntr_no) {
		...
		fetch(`${contextPath}/${siteId}/agreement/request.do?mId=52`,{
			method: 'POST',
			body: form_data
		}).then(e => {
			console.log(e)
	...
	function download(idx) {
		...
		if(newUrl.includes('/taskhub')) {
			newUrl = newUrl.split('/taskhub')[0] + '/agreement/download.do?mId=52'
		}
