...
// 서식 파일 추가 
OOFFile file = oof.addFile("crf.root", "%root%\\crf\\areaAct.crf");


//매개변수 필드 추가 (arg1:매개변수명, arg2:값)
//기초진단지 IDX 가져오기
String aractplnIdx = (String) request.getAttribute("aractplnIdx");
//oof.addField("param1", "value");
//oof.addField("BSC_IDX", bscIdx);
/* String[] aracList = (String[]) request.getAttribute("aracList"); */
oof.addField("ARACTPLN_IDX", aractplnIdx);

/* int splitPage = aracList.length;
String[] fileNames = new String[splitPage]; */
...
String fileName = uploadPath + "/areaAct/report/areaAct_" + bscIdx + ".pdf";
File localFileSave = new File(fileName);
OutputStream fileStream = new FileOutputStream(localFileSave); */

//oof.addOutputPDF("D:\\Project\\doctor\\Source\\src\\main\\webapp\\upload\\rbs\\diagnosis\\report");
/* Data Connection */
...
<script>
	...
	function html2xml(divPath){
		...
		var report = createReport(urlPath + "${pageContext.request.contextPath}/include/js/ClipReport5/report_server.jsp", reportkey, document.getElementById(divPath));

		report.view();
		//report.setDefaultSavePDFOption("", true, true, false);
		//report.saveExportByType(3, 1, 1, "test", report.objSaveExportOptionInfo.pdf);
		console.log(report);
	}
</script>
</head>
...
<div id='targetDiv1' style='position:absolute;top:5px;left:5px;right:5px;bottom:5px;'>
	<span style="visibility: hidden; font-family:나눔고딕">.${contextPath} /// </span>
