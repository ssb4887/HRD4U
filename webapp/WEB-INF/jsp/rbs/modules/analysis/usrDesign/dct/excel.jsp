	...
	<table style="width:100%;">
		...
		<tbody>
			...
			<tr>
				...
				<td><c:out value="${listDt.TR_CORP_NM}"/></td>
				<td><c:out value="${listDt.TP_NM}"/></td>
				<td><c:out value="${listDt.CNSL_IDX eq null ? '과정개발' : '컨설팅'}"/></td>
				<td><c:out value="${listDt.TR_RESULT_REPRT_SE eq '1' ? '만족도 및 성취도 조사' : '현업적용'}" /></td>
				<td><fmt:formatDate pattern="yyyy-MM-dd" value="${listDt.REGI_DATE}"/></td>
				<td><c:out value="${listDt.STATUS eq '0' ? '작성중' : '제출'}"/></td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
