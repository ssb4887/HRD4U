...
$(function(){
	...
	$("#<c:out value="${param.inputFormId}"/> .fn_btn_submit").click(function(){
		try {
			...
			var statusName = "";
			$("#fn_supportInputForm").attr("action", "${DEVELOP_SUPPORT_MODIFY_URL}");

			
			var varConfirm = confirm("맞춤훈련과정개발 맟춤개발 지원금 신청서를  수정 하시겠습니까?");
			if(!varConfirm) return false;
...
function fn_requestInputFormSubmit(confmStatus){
	...
	$("tr[data-type='in']").each(function() {
		...
		for(var i = 0; i < subInputList.length; i++){
			if(!$("#" + subInputList[i] + id).val()){
				alert("내부훈련교사 " + subInputName[i] + " 를 입력해 주세요.");
				$("#" + subInputList[i] + id).focus();
				return false;
			}
		}
	});
