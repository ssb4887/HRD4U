...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/develop/default/view.jsp"/>
	</jsp:include>
</c:if>
...
<c:set var="itemObjs" value="${itemInfo.items}"/>
	<div id="cms_board_article">
		...
		<div class="btnList">
			<span class="btnBc"><a href="<c:out value="${URL_LIST}"/>" class="fn_btn_list">목록</a></span>
			<c:if test="${mngAuth || wrtAuth && dt.AUTH_MNG == '1'}">
			<span class="btnBc orange"><a href="<c:out value="${URL_IDX_MODIFY}"/>" class="fn_btn_modify">수정</a></span>
			<span class="btnBc gray"><a href="<c:out value="${URL_IDX_DELETEPROC}"/>" class="fn_btn_delete">삭제</a></span>
			</c:if>
		</div>
	</div>
