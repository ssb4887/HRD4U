...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/develop/develop/trainingList.jsp"/>
	...
	<div class="contents-area">
		...
		<div class="title-wrapper">
			...
			<div class="fr">
				...
				<a href="#none" class="btn-m01 btn-color01" id="open-modal02">신규 등록</a>
			</div>
			</c:if>
		</div>
...
<%-- 					<c:set var="listNo" value="${paginationInfo.totalRecordCount - paginationInfo.firstRecordIndex}" /> --%>
					...
					<tr>
						...
						<strong class="point-color01">
                        	 <a href="${DEVELOP_TRAINING_VIEW_FORM_URL}&tpIdx=${listDt.TP_IDX}&prtbizIdx=${listDt.PRTBIZ_IDX}&source=${listDt.SOURCE}&isFromDevelop=${isFromDevelop}"><c:out value="${listDt.TP_NAME}"/></a>
						</strong>
						</td>
						<td>
                        ...
                        <c:if test="${isFromDevelop eq 'Y' }">
                        <td>
                        	 <a href="${DEVELOP_TRAINING_VIEW_FORM_URL}&tpIdx=${listDt.TP_IDX}&prtbizIdx=${listDt.PRTBIZ_IDX}&source=${listDt.SOURCE}&isFromDevelop=${isFromDevelop}" class="btn-m02 btn-color03">선택</a> 
                        </td>	
                        </c:if>
					</tr>
...
<div class="modal-wrapper" id="modal-action02">
	...
	<div class="modal-area">
		...
		<div class="btns-area">
			<a href="<c:out value="${DEVELOP_WRITE_FORM_URL}"/>&useFlag=new&prtbizIdx=4"  class="btn-m01 btn-color03 depth2 fn_btn_submit">사업주</a>
			<a href="<c:out value="${DEVELOP_WRITE_FORM_URL}"/>&useFlag=new&prtbizIdx=1" class="btn-m01 btn-color03 depth2 fn_btn_submit">S-OJT</a>				
		</div>
	</div>
