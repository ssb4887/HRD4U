...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/develop/develop/trainingView.jsp"/>
	</jsp:include>
</c:if>
		...
		<div class="contents-box pl0">
			<div class="table-type02 horizontal-scroll">
            	<table class="width-type02">
            		<caption>능력개발클리닉 참여 신청서 세부내용 폼</caption>
            		<colgroup>
                        <col width="15%">
                        <col width="35%">
                        <col width="15%">
...
<%-- 				                    		<td>${tpSubList[i.index].COURSE_NO }</td> --%>
		...
		<div class="btns-area">
			<div class="btns-right">
				<c:if test="${isFromDevelop eq 'Y' }">
				<a href="<c:out value="${DEVELOP_WRITE_FORM_URL}"/>&tpIdx=${tpList.TP_IDX}&prtbizIdx=${tpList.PRTBIZ_IDX }&useFlag=use" class="btn-m01 btn-color03 depth2 fn_btn_modify">추천 활용</a>						
				<a href="<c:out value="${DEVELOP_WRITE_FORM_URL}"/>&tpIdx=${tpList.TP_IDX}&prtbizIdx=${tpList.PRTBIZ_IDX }&useFlag=reuse" class="btn-m01 btn-color03 depth2 fn_btn_modify">과정 수정</a>
				</c:if>
