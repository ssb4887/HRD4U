...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page"
			value="${moduleJspRPath}/develop/develop/recommendView.jsp" />
	</jsp:include>
</c:if>
											...
											<c:forEach items="${trainingRecommend}" varStatus="j">
												<c:if test="${tpCount lt 3 && trainingRecommend[i.index].RANK eq trainingRecommend[j.index].RANK}">
													<c:if test="${!empty trainingRecommend[j.index].TP_NAME}">
		                               					<a href="${DEVELOP_TRAINING_VIEW_FORM_URL}&tpIdx=${trainingRecommend[j.index].TP_IDX}&prtbizIdx=${trainingRecommend[j.index].PRTBIZ_IDX}&isFromDevelop=Y" class="point-color01 btn-linked" style="display:inline;">
						...
						<%-- <tr>
							<c:forEach items="${trainingRecommend }" varStatus="i">
								<c:if
									...
									<c:if test="${!empty trainingRecommend[i.index].TP_IDX }">
										<td><a href="${DEVELOP_TRAINING_VIEW_FORM_URL}&tpIdx=${trainingRecommend[i.index].TP_IDX}&prtbizIdx=${trainingRecommend[i.index].PRTBIZ_IDX}" class="btn-m01 btn-color03">과정 상세보기</a></td>
									</c:if>
								</c:if>
							</c:forEach>
						</tr> --%>
...
<%-- 			<a href="${DEVELOP_TRAINING_LIST_FORM_URL}&tpList=${tpList}&isFromDevelop=Y" class="word-linked-notice03"> --%>
			<a href="${DEVELOP_TRAINING_LIST_FORM_URL}&isFromDevelop=Y" class="word-linked-notice03">
				희망하시는 훈련과정이 없으신가요? <span class="span-mobie-br"></span><span class="point-color01 underline">훈련과정 목록</span>에서 직접 선택할 수 있습니다.<img src="${contextPath}${imgPath}/icon/icon_arrow_right04.png">
			</a>
		</div>
	</div>
</div>
