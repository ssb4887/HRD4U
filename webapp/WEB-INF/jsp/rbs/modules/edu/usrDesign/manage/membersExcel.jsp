...
String titleName = "교육과정_신청현황_" + formattedDate + "";
...
<body topmargin="0" leftmargin="0" marginheight="0" marginwidth="0">
	...
	<table summary="<%=titleName%> 목록을 제공합니다." style="width:100%;">
		<caption>(<c:out value='${edc.EDC_NAME}' />)<%=titleName%></caption>
		<colgroup>
			<col />
			<col />
			<col />
		...
		<tbody class="alignC">
			<c:if test="${empty list}">
				<tr>
					<td colspan="10" class="nolist"><spring:message code="message.no.list"/></td>
				</tr>
			</c:if>
			<c:forEach var="listDt" items="${list}" varStatus="i">
				<tr>
					...
					<td style="mso-number-format:'@';"><c:out value='${listDt.MOBILE_PHONE}' /></td>
					<td><c:out value='${listDt.CONFM_STATUS_NAME}' /></td>
					<td>
						<c:if test="${listDt.ATT_YN eq 'Y'}">참석</c:if>
						<c:if test="${listDt.ATT_YN eq 'N'}">불참석</c:if>
					</td>
				</tr>
			</c:forEach>
		</tbody>
