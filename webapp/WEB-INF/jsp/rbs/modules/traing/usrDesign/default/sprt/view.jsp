...
<c:set var="itemObjs" value="${itemInfo.items}"/>
	<div id="cms_board_article" class="contents-wrapper">
		<h3 class="title-type02 h3_textbox2">최근 직업훈련 지원 현황 상세보기</h3>
				...
				<c:set var="summary"><itui:tableSummary items="${items}" itemOrder="${itemOrder}" exceptIds="${exceptIds}"/> 입력표</c:set>
					...
					<table id="tableSprt">
						...
						<tbody>
							...
							<itui:objectInputView indutyCd="${indutyCd}" year="${year}" objDt="${dt}" selectClass="sprt" tableId="tableSprt" idx="2" submitType="${submitType}"/>
						</tbody>
					</table>
					</div>
					<div class="btns-area mt60">
						...
						<a href="${contextPath}/${crtSiteId}/traing/list.do?mId=40" class="btn-m01 btn-color01">목록</a>
					</div>
			</div>
		</div>
	</div>
