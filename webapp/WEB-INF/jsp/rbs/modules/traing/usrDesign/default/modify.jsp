...
<c:if test="${!empty TOP_PAGE}">
	...
	<div id="cms_board_article" class="contents-wrapper">
		<h3 class="title-type02 h3_textbox">최근 직업훈련 실시 현황 수정</h3>
				...
				<c:set var="summary"><itui:tableSummary items="${items}" itemOrder="${itemOrder}" exceptIds="${exceptIds}"/> 입력표</c:set>
					...
					<table id="tableTraing">
						...
						<tbody>
							...
							<itui:objectInputModify indutyCd="${indutyCd}" year="${year}" objDt="${list}" selectClass="traing" tableId="tableTraing" idx="1" submitType="${submitType}"/>
						</tbody>
					</table>
					</div>
