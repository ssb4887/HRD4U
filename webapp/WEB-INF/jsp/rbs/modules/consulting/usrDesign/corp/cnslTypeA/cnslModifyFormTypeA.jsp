...
<div class="contents-area pl0">
				...
				<tbody>
					...
					<tr>
						<td class="left">
							<div class="input-radio-area">
								<input type="radio" id="radio0101" name="cnslType" value="1" class="radio-type01" onclick="sojtLimitCheck(this, `${cnsl.bplNo}`)" <c:if test="${cnsl.cnslType eq 1}">checked</c:if> data-checkValidity="훈련과정 유형" required>
								<label for="radio0101"></label>
							</div>
						</td>
						<td class="left">
							<div class="input-radio-area">
									<input type="radio" id="radio0102" name="cnslType" value="2" class="radio-type01" onclick="sojtLimitCheck(this, `${cnsl.bplNo}`)" <c:if test="${cnsl.cnslType eq 2}">checked</c:if> data-checkValidity="훈련과정 유형" required> 
									<label for="radio0102"></label>
							</div>
						</td>
						<td class="left">
							<div class="input-radio-area">
									<input type="radio" id="radio0103" name="cnslType" value="3" class="radio-type01" onclick="sojtLimitCheck(this, `${cnsl.bplNo}`)" <c:if test="${cnsl.cnslType eq 3}">checked</c:if> data-checkValidity="훈련과정 유형" required> 
					...
					<tr>
						...
						<td class="left" colspan="3"><textarea id="textfield01"
								name="cnslDemandMatter" cols="50" rows="5" class="w100 h100" data-checkValidity="훈련직무 및 개발 요구사항" requried><c:out value="${cnsl.cnslDemandMatter}"/></textarea></td>
					</tr>
					<tr>
						<th scope="row">직무분류 <strong class="point-important">*</strong></th>
						<td class="left" colspan="3">
							<div class="input-add-btns">
								<input type="hidden" id="ncsCode" name="dtyCl" value="${cnsl.dtyCl}" class="" placeholder="" data-checkValidity="직무분류" required>
								<input type="text" id ="ncsName" class="" value="${cnsl.dtyClNm}" placeholder="">
				...
				<tbody>
					...
					<tr>
						...
						<td class="left">
								...
								<option value="경기경영자총협회" <c:if test="${cnsl.recomendInsttNm eq '경기경영자총협회'}">selected</c:if>>경기경영자총협회</option>
								<option value="대한상공회의소 경기인력개발원" <c:if test="${cnsl.recomendInsttNm eq '대한상공회의소 경기인력개발원'}">selected</c:if>>대한상공회의소 경기인력개발원</option>
								<option value="한국공학대학교" <c:if test="${cnsl.recomendInsttNm eq '한국공학대학교'}">selected</c:if>>한국공학대학교</option>
								<option value="대한상공회의소 충남인력개발원" <c:if test="${cnsl.recomendInsttNm eq '대한상공회의소 충남인력개발원'}">selected</c:if>>대한상공회의소 충남인력개발원</option>
								<option value="한국문화산업협회" <c:if test="${cnsl.recomendInsttNm eq '한국문화산업협회'}">selected</c:if>>한국문화산업협회</option>
								<option value="전북산학융합원" <c:if test="${cnsl.recomendInsttNm eq '전북산학융합원'}">selected</c:if>>전북산학융합원</option>
								<option value="경남경영자총협회" <c:if test="${cnsl.recomendInsttNm eq '경남경영자총협회'}">selected</c:if>>경남경영자총협회</option>
								<option value="대한상공회의소 부산인력개발원" <c:if test="${cnsl.recomendInsttNm eq '대한상공회의소 부산인력개발원'}">selected</c:if>>대한상공회의소 부산인력개발원</option>
								<option value="경북경영자총협회" <c:if test="${cnsl.recomendInsttNm eq '경북경영자총협회'}">selected</c:if>>경북경영자총협회</option>
								</select>
						</td>
					</tr>
					<tr>
						...
						<td class="left">
							<input type="text" id="cmptncBrffcNm" value="${cnsl.cmptncBrffcNm}" class="w100" readOnly>
							<input type="hidden" id="cmptncBrffcIdx" name="cmptncBrffcIdx" value="${cnsl.cmptncBrffcIdx}" class="w100" data-checkValidity="관할 지부지사" required>
							<input type="hidden" id="cmptncBrffcPicIdx" name="cmptncBrffcPicIdx" value="${cnsl.cmptncBrffcPicIdx}" class="w100" required>
						</td>
					</tr>
					<tr>
						<th scope="row">연락처</th>
						<td class="left"><input type="text" id="cmptncBrffcPicTelno" name="cmptncBrffcPicTelno"
							value="${cnsl.cmptncBrffcPicTelno}" class="w100" readOnly></td>
					</tr>
				</tbody>
			</table>
		</div>
				...
				<tbody>
					...
					<tr>
						...
						<td class="left"><input type="text" id="corpPicNm"
							name="corpPicNm" value="${cnsl.corpPicNm}" class="w100" data-checkValidity="담당자 성명" required></td>
						<th scope="row">직위  <strong class="point-important">*</strong></th>
						<td class="left"><input type="text" id="corpPicOfcps"
							name="corpPicOfcps" value="${cnsl.corpPicOfcps}" class="w100" data-checkValidity="담당자 직위" required></td>
					</tr>
					<tr>
						<th scope="row">전화번호  <strong class="point-important">*</strong></th>
						<td class="left"><input type="text" id="corpPicTelno"
							name="corpPicTelno" value="${cnsl.corpPicTelno}" class="w100" data-checkValidity="담당자 전화번호" required></td>
						<th scope="row">E-mail <strong class="point-important">*</strong></th>
						<td class="left"><input type="text" id="corpPicEmail"
							name="corpPicEmail" value="${cnsl.corpPicEmail}" class="w100" data-checkValidity="담당자 E-mail" required></td>
					</tr>
				</tbody>
			</table>
		</div>
	...
	<div class="contents-box pl0">
		<div class="table-type02 horizontal-scroll">
			<table class="width-type02" id="teamTable">
				...
				<tbody>
					<tr>
						<th scope="col" colspan="1"><input type="checkbox" id="teamCheck" name="teamCheck" onchange="checkTeamCreationStatus()">팀구성 추천요망</th>
						<th scope="col" colspan="4">3. 컨설팅 팀 구성 및 개요</th>
					</tr>
					<tr>
					...
					<tr class="pm">
						...
						<td class="mberIdTd">
							<input type="text" id="pmId" name="cnslTeam.members[0].loginId" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 1 and member.teamOrderIdx eq 1}">${member.loginId}</c:if></c:forEach>" class="w100" onblur="hrpUserSearchHandler(this)" data-checkValidity="컨설팅 책임자 (PM)" required></td>
						<td class="">
					...
					<tr class="exExpert" id="exExpert_01">
						<th scope="row">외부 내용전문가
						</th>
						<td class="mberIdTd">
						 <input type="text" id="exExpertId_01" name="cnslTeam.members[1].loginId" onblur="hrpUserSearchHandler(this)" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 2 and member.teamOrderIdx eq 1}">${member.loginId}</c:if></c:forEach>" class="w100" data-checkValidity="외부 내용전문가" required></td>
						<td class="">
						 <input type="text" id="exExpertId_01_name" name="cnslTeam.members[1].mberName" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 2 and member.teamOrderIdx eq 1}">${member.mberName}</c:if></c:forEach>" class="w100" required></td>
						<td class="checkSuitableRadioTd">
					...
					<tr class="inExpert" id="inExpert_01">
						...
						<td class="mberIdTd">
							<input type="text" id="inExpertId_01" name="cnslTeam.members[4].loginId" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 3 and member.teamOrderIdx eq 1}">${member.loginId}</c:if></c:forEach>" class="w100" placeholder="HRD4U ID" 	onblur="userSearchHandler(this)" data-checkValidity="기업 내부전문가" required></td>
						<td class="" colspan="2">
							<input type="text" id="inExpertId_01_name" name="cnslTeam.members[4].mberName" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 3 and member.teamOrderIdx eq 1}">${member.mberName}</c:if></c:forEach>" class="w100" placeholder="성명"></td>
						<td class="">
						...
						<td class="left">
							<div class="fileBox">
								<input type="text" id="fileName_01" class="fileName"
									readonly="readonly" placeholder="파일찾기" value="<c:forEach var="file" items="${cnsl.cnslFiles}"><c:if test="${file.itemId eq 'pmFile'}">${file.fileOriginName}</c:if></c:forEach>" data-checkValidity="컨설팅 책임자(PM) 첨부파일" required> <label
						...
						<td class="left">
							<div class="fileBox">
								<input type="text" id="fileName_02" class="fileName"
									readonly="readonly" placeholder="파일찾기" value="<c:forEach var="file" items="${cnsl.cnslFiles}"><c:if test="${file.itemId eq 'exFile_01'}">${file.fileOriginName}</c:if></c:forEach>" data-checkValidity="외부 내용전문가 첨부파일" required> <label
						...
						<td class="left">
							<div class="fileBox">
								<input type="text" id="fileName_03" class="fileName"
									readonly="readonly" placeholder="파일찾기" value="<c:forEach var="file" items="${cnsl.cnslFiles}"><c:if test="${file.itemId eq 'inFile_01'}">${file.fileOriginName}</c:if></c:forEach>" data-checkValidity="기업 내부전문가 첨부파일" required> <label
	...
	<div class="contents-box pl0" style="
    margin-bottom: 40px;">
		<div class="table-type02 horizontal-scroll">
			<table class="width-type02">
				<colgroup>
