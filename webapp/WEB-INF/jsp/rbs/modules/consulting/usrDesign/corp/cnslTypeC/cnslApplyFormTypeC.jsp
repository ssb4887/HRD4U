...
<div class="contents-area pl0">
	...
	<h3 class="title-type01 ml0">심화컨설팅 추가 신청서</h3>
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3" class="left">
							<input type="text" id="trOprtnRegionZip" name="trOprtnRegionZip" value="${cnsl.trOprtnRegionZip}" class="w100" readOnly>
							<input type="text" id="trOprtnRegionAddr" name="trOprtnRegionAddr" value="${cnsl.trOprtnRegionAddr}" class="w100"readOnly>
							<input type="text" id="trOprtnRegionAddrDtl" name="trOprtnRegionAddrDtl" value="${cnsl.trOprtnRegionAddrDtl}" class="w100" readOnly>
						</td>
					</tr>
					...
					<tr>
						...
						<td class="left">
							...
							<input type="hidden" id="cmptncBrffcIdx" name="cmptncBrffcIdx" value="${cnsl.cmptncBrffcIdx}" class="w100" readOnly>
				...
				<tbody>
					...
					<tr>
						...
						<td class="left"><input type="text" id="corpPicTelno"
							name="corpPicTelno" value="${cnsl.corpPicTelno}" class="w100" required></td>
	...
	<div class="contents-box pl0">
		<div class="table-type02 horizontal-scroll">
			<table class="width-type02">
				...
				<tbody>
					<tr>
						<th scope="col" colspan="3">3. 컨설팅 유형</th>
					</tr>
					<tr>
						<th scope="row" rowspan="2">훈련과정 유형</th>
						<th scope="row">훈련체계수립컨설팅</th>
						<th scope="row">현장활용컨설팅</th>
					</tr>
					<tr>
						<td class="left">
							<div class="input-radio-area">
								<input type="radio" id="cnslType5" name="cnslType" value="5" class="radio-type01" onclick="checkLimitCnslType(`${cnsl.bplNo}`, this)"  required> 
								<label for="radio0101"></label>
							</div>
						</td>
						<td class="left">
							<div class="input-radio-area">
									<input type="radio" id="cnslType6" name="cnslType" value="6" class="radio-type01" onclick="checkLimitCnslType(`${cnsl.bplNo}`, this)" required> 
	...
	<div class="contents-box pl0">
		<div class="table-type02 horizontal-scroll">
			<table class="width-type02" id="teamTable">
				...
				<tbody>
					<tr>
						<th scope="col" colspan="9">4. 컨설팅 팀 구성 및 개요</th>
					</tr>
					<tr>
					...
					<tr class="exExpert" id="exExpert_01">
						...
						<td class="">
							...
							<input type="hidden" name="cnslTeam.members[1].rspnberYn" value="N"> 
							<input type="hidden" name="cnslTeam.members[1].teamIdx" value="2"> 
							<input type="hidden" name="cnslTeam.members[1].teamOrderIdx" value="1">
							<input type="hidden" id="exExpertId_01_memberIdx" name="cnslTeam.members[1].memberIdx" value="<c:forEach var="member" items="${cnsl.cnslTeam.members}"><c:if test="${member.teamIdx eq 1 and member.teamOrderIdx eq 1}">${member.memberIdx}</c:if></c:forEach>">
					...
					<c:forEach var="member" items="${cnsl.cnslTeam.members}" varStatus="status">
						...
						<td class="">
						 <input type="text" id="exExpertId_0${member.teamOrderIdx}_name" name="cnslTeam.members[${member.teamOrderIdx}].mberName" value="${member.mberName}" class="w100" required>
							<input  type="hidden" name="cnslTeam.members[${member.teamOrderIdx}].rspnberYn" value="N"> 
							<input  type="hidden" name="cnslTeam.members[${member.teamOrderIdx}].teamIdx" value="2"> 
							<input  type="hidden" name="cnslTeam.members[${member.teamOrderIdx}].teamOrderIdx" value="${member.teamOrderIdx}">
		...
		<div class="table-type02 horizontal-scroll">
			<table class="width-type02" id="attachedTable">
				...
				<tbody>
					...
					<tr>
						<td class="">기타</td>
						<td class="left"><strong class="point-color01"> 
						<c:forEach var="file" items="${cnsl.cnslFiles}">
						<c:if test="${file.itemId eq 'etcFile'}">${file.fileOriginName}
						</strong> <a
							href="${contextPath}/web/consulting/download.do?mId=85&fileName=<c:forEach var="file" items="${cnsl.cnslFiles}"><c:if test="${file.itemId eq 'etcFile'}">${file.fileSavedName}</c:if></c:forEach>"
							class="btn-linked"> <img
								src="${contextPath}/web/images/icon/icon_search04.png" alt="" />
						</a>
						</c:if>
