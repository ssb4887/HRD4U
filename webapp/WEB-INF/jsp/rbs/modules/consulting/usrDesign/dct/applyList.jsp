...
<article>
	<div class="contents" id="contents">
					...
					<table>
						...
						<tbody class="alignC">
							<c:if test="${empty list}">
								<tr>
									<td colspan="10" class="bllist"><spring:message
							...
							<c:forEach var="listDt" items="${list}" varStatus="i">
								...
								<tr>
									...
									<td>
										<c:choose>
											<c:when test="${listDt.CONFM_STATUS eq 10}">
												<a href="${contextPath}/dct/consulting/cnslViewForm.do?mId=124&cnslIdx=${listDt.CNSL_IDX}" />				
											</c:when>
											<c:otherwise>
												<a href="${contextPath}/dct/consulting/cnslAdminView.do?mId=124&cnslIdx=${listDt.CNSL_IDX}" />
											</c:otherwise>
										</c:choose>
									...
									<td>
										<itui:objectView itemId="cnslTme"
												itemInfo="${itemInfo}" objDt="${listDt}" />
									</td>
									...
									<td><c:choose>
											...
											<c:otherwise>
												<button type="button" class="btn-m02 btn-color03" 
													onClick="location.href='${contextPath}/dct/consulting/cnslAdminView.do?mId=124&cnslIdx=${listDt.CNSL_IDX}'">상세보기</button>
											</c:otherwise>
										</c:choose></td>
									<td>
										<button type="button" class="btn-m02 btn-color03"
											onClick="window.open('${contextPath}/dct/bsisCnsl/consultingClipReport.do?mId=37&bsiscnslIdx=${listDt.BSISCNSL_IDX}', '_blank')">
											<span> 열기 </span>
										</button>
									</td>
								</tr>
		...
		<div class="paging-navigation-wrapper">
			<pgui:pagination listUrl="applyList.do?mId=124" pgInfo="${paginationInfo}" imgPath="${imgPath}" pageName="${elfn:getString(settingInfo.page_name, 'page')}"/>
			</div>
			</div>
