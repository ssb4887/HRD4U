...
<div class="contents-area">
	<div class="title-wrapper">
		...
		<div class="fr">
	    	<a href="${contextPath}/dct/consulting/cnslExcel.do?mId=126
	    	<c:if test="${not empty param.is_bplNo}">&is_bplNo=${param.is_bplNo}</c:if>
	    	<c:if test="${not empty param.is_cmptncBrffcIdx}">&is_cmptncBrffcIdx=${param.is_cmptncBrffcIdx}</c:if>
	    	<c:if test="${not empty param.is_confmStatus}">&is_confmStatus=${param.is_confmStatus}</c:if>
	    	<c:if test="${not empty param.is_cnslType}">&is_cnslType=${param.is_cnslType}</c:if>" class="btn-m01 btn-color01">엑셀다운로드</a>
	    </div>
	</div>
			...
			<tbody class="alignC">
				...
				<c:forEach var="listDt" items="${list}" varStatus="i">
					<tr>
						...
						<td><a
							href="${contextPath}/dct/consulting/viewAll.do?mId=126&cnslIdx=${listDt.cnslIdx}" />
						...
						<td>
						<c:choose>
							<c:when test="${listDt.confmStatus eq 40}"><span onclick="openModal('reject-modal', `${listDt.cnslIdx}`)"><strong class="point-color03"> 반려</strong></span></button></c:when>
						...
						<td><c:choose>
								<c:when test="${listDt.confmStatus eq 70}">
									<a
										href="${contextPath}/dct/consulting/viewAll.do?mId=126&cnslIdx=${listDt.cnslIdx}#tab2"><strong
								...
								<c:when test="${listDt.confmStatus eq 75}">
									<a
										href="${contextPath}/dct/consulting/viewAll.do?mId=126&cnslIdx=${listDt.cnslIdx}#tab1"><strong
						...
						<td>
							<c:choose>
								...
								<c:otherwise>
									<a href="${contextPath}/dct/consulting/viewAll.do?mId=126&cnslIdx=<c:out value="${listDt.cnslIdx}"/>&bplNo=<c:out value="${listDt.bplNo}"/>#tab3"><strong class="point-color01"> 이동</strong></a>
								</c:otherwise>
						...
						<td>
							<c:choose>
								<c:when test="${listDt.confmStatus eq 40}"><strong class="point-color01">-</strong></c:when>
								<c:otherwise>
									<a href="${contextPath}/dct/consulting/viewAll.do?mId=126&cnslIdx=<c:out value="${listDt.cnslIdx}"/>&bplNo=<c:out value="${listDt.bplNo}"/>#tab4"><strong
						...
						<td>
							<c:choose>
								<c:when test="${listDt.confmStatus eq 40 or listDt.cnslType eq '1' or listDt.cnslType eq '2'}"><strong class="point-color01">-</strong></c:when>
								<c:otherwise>
									<a href="${contextPath}/dct/consulting/evalList.do?mId=127&cnslIdx=<c:out value="${listDt.cnslIdx}"/>&bplNo=<c:out value="${listDt.bplNo}"/>"><strong class="point-color01"> 이동</strong></a>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
	...
	<div class="paging-navigation-wrapper">
		<pgui:pagination listUrl="cnslListAll.do?mId=126"
