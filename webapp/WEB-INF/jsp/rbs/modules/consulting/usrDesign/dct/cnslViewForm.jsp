...
<c:choose>
	<c:when
		...
		<%@ include file="cnslForm/cnslViewFormTypeA.jsp"%>
	</c:when>
	<c:when test="${cnsl.cnslType eq '4'}">
		<%@ include file="cnslForm/cnslViewFormTypeB.jsp"%>
	</c:when>
</c:choose>
...
<div class="btns-area">
	...
	<div class="btns-right">
		...
		<c:choose>
			<c:when test="${siteId eq 'web'}">
						<a href="/web/consulting/applyList.do?mId=96"
			class="btn-m01 btn-color01 depth3"> 목록 </a>
			</c:when>
			<c:when test="${siteId eq 'dct'}">
							
					<a href="/dct/consulting/applyList.do?mId=124"
			class="btn-m01 btn-color01 depth3"> 목록 </a>
			</c:when>
		</c:choose>
