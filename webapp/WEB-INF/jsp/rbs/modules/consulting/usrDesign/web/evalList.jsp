					...
					<c:forEach var="listDt" items="${list}" varStatus="i">
						...
						<tr id="column">
							...
							<td>
								...
								<a href="evalInput.do?mId=94&cnslIdx=${listDt.CNSL_IDX}&evlIdx=${listDt.EVL_IDX}<c:if test="${not empty listDt.EVL_IDX}" >&mode=m</c:if>" class="btn-m02 btn-color03">
                                 	평가: <c:out value="${listDt.EVL_GRAD}" />
                            	</a>
                            	</c:if>
                            	<c:if test="${loginVO.usertypeIdx ne '20'}">
                            		평가: <c:out value="${listDt.EVL_GRAD}" />
                            	</c:if>
							</td>
						</tr>
		...
		<div class="paging-navigation-wrapper">
			<form action="list.do" name="pageForm" method="get">
				...
				<p class="paging-navigation" onclick="paginationHandler('pageForm')">
					<pgui:pagination listUrl="${contextPath}/web/consult/consultant.do?mId=${mId}" pgInfo="${paginationInfo}" imgPath="${imgPath}" pageName="${elfn:getString(settingInfo.page_name, 'page')}" />
				</p>
				<!-- //페이징 네비게이션 -->
			</form>
		</div>
