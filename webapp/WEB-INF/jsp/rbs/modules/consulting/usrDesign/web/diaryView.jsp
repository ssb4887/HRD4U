		...
		<h3 class="title-type02 ml0">컨설팅 수행일지</h3>
					...
					<table>
						...
						<tbody>
							<tr>
								<th scope="row">기업명</th>
								<td>
									<itui:objectView itemId="bplNm" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
								<th scope="row">컨설팅 유형</th>
								<td>
									<c:out value="${dt.SPORT_TYPE eq '1' ? '사업주자체 과정' : dt.SPORT_TYPE eq '2' ? '일반직무전수 OJT' : dt.SPORT_TYPE eq '3' ? '과제수행 OJT' : dt.SPORT_TYPE eq '4' ? '심층진단' : dt.SPORT_TYPE eq '5' ? '훈련체계 수립' : dt.SPORT_TYPE eq '6' ? '현장활용' : ''}" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="left" colspan="3">
									<itui:objectView itemId="excOdr" itemInfo="${itemInfo}" objDt="${dt}" />차
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td colspan="2" class="left">
									<itui:objectView itemId="pmNm" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<th scope="row" class="bg01">내용전문가</th>
								<td colspan="2" class="left">
									<itui:objectView itemId="cnExpert" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<th scope="row" class="bg01">기업 내부전문가</th>
								<td colspan="2" class="left">
									<itui:objectView itemId="corpInnerExpert" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<th scope="row" class="bg01">기타 참석자(기업 등)</th>
								<td colspan="2" class="left">
									<itui:objectView itemId="spntPic" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<th scope="row" rowspan="2">수행내용</th>
								<td colspan="3" class="left">
									<itui:objectView itemId="mtgWeekExplsn1" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<td colspan="3" class="left">	
									<itui:objectTextarea itemId="mtgCn1" itemInfo="${itemInfo}" objDt="${dt}" />
								</td>
							</tr>
							<tr>
								<th>회의사진</th>
								<td colspan="3" class="left">
									<c:if test="${not empty multiFileHashMap.photo}">
										<c:forEach begin="0" end="${fn:length(multiFileHashMap.photo)-1}" varStatus="i">
											<img src='<c:url value='${contextPath}/web/consulting/diaryDownload.do'/>?mId=99&diaryIdx=${dt.DIARY_IDX}&fidx=<c:out value="${not empty multiFileHashMap.photo[i.index].FLE_IDX ? multiFileHashMap.photo[i.index].FLE_IDX : 0}" />&itld=photo' alt="회의사진"/>
										</c:forEach>
									</c:if>
								</td>
							</tr>
 		...
 		<div class="btns-area" id="btns-area-1" style="margin-top: 30px;">
 			<div class="btns-right">
	            <button type="button" class="btn-m01 btn-color01" onclick="history.back();" >
	            	목록
	            </button>
	            <button type="button" class="btn-m01 btn-color02 depth2" onclick="deleteDiary();" >
	            	삭제
	            </button>
 			</div>
        </div>
