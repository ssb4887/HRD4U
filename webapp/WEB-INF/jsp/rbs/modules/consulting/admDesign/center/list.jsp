...
<div class="contents-area">
		...
		<div class="table-type02 horizontal-scroll">
			<table class="listTypeA"
				...
				<tbody class="alignC">
					...
					<c:if test="${empty list || totalCount eq 0}">
						<tr>
					...
					<c:forEach var="listDt" items="${list}" varStatus="i">
						...
						<tr>
							...
							<td class="subject"><c:choose>
									<c:when test="${listDt.STATUS eq 0 or listDt.STATUS eq 4}">
										<a
											href="applyForm.do?mId=85&bplNo=${listDt.BPL_NO}&bsiscnslIdx=${listDt.BSISCNSL_IDX}&cnslType=${listDt.CNSL_TYPE}&cnslIdx=${listDt.CNSL_IDX}" />
									</c:when>
									<c:otherwise>
										<a href="<c:out value="${URL_VIEW}"/>&${listIdxName}=${listKey}"
											class="fn_btn_view">
									</c:otherwise>
								</c:choose> <itui:objectView itemId="corpNm" itemInfo="${itemInfo}"
									objDt="${listDt}" /></a></td>
							<td><itui:objectView itemId="cnslTme" itemInfo="${itemInfo}"
									objDt="${listDt}" /></td>
							...
							<td><c:choose>
									<c:when test="${listDt.STATUS eq 1}">
										<button type="button" onclick="cancelCnsl(`${listDt.CNSL_IDX}`,`4`)">회수</button>
									</c:when>
									<c:when test="${listDt.STATUS eq 2}">
										<button type="button">반려요청</button>
									</c:when>
								</c:choose></td>
						</tr>
	...
	<div class="btnList">
		<div class="btns-right">
			<a
				href="${contextPath}/web/consulting/apply.do?mId=85&bplNo=${bplNo}&bsiscnslIdx=${bsiscnslIdx}"
				class="btn-m01 btn-color01">신청서 작성</a>
		</div>
	</div>
</div>
