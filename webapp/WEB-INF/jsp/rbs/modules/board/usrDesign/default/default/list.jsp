...
<c:if test="${!empty TOP_PAGE}">
	...
	<div id="cms_board_article">
		...
		<!-- search -->
		<itui:searchFormItemForBoard formId="${searchFormId}" formName="${searchFormId}" formAction="${URL_DEFAULT_LIST}" itemListSearch="${itemInfo.list_search}" searchOptnHashMap="${searchOptnHashMap}" searchFormExceptParams="${searchFormExceptParams}" submitBtnId="fn_btn_search" submitBtnClass="btnSearch" submitBtnValue="검색" listAction="${URL_DEFAULT_LIST}" listBtnId="fn_btn_totallist" listBtnClass="btnTotalList"/>
		<!-- //search -->
		...
		<table>
			...
			<colgroup>
				<col width="6%" />
				<col />
				<c:if test="${useQna}"><col width="6%" /></c:if>
				<c:if test="${useFile}"><col width="6%" /><c:set var="colSpan" value="${colSpan + 1}"/></c:if>
				<col width="12%" />
				<col width="12%" />
				<c:if test="${!useQna}"><col width="6%" /></c:if>
			</colgroup>
			...
			<tbody class="alignC">
				...
				<tr<c:if test="${isNotice}"> style="background-color:#f9f0f9;"</c:if>>
					...
					<c:if test="${!useQna}"><td class="hit" id="fn_brd_views<c:out value="${listDt.BRD_IDX}"/>"><c:out value="${listDt.VIEWS}"/></td></c:if>
				</tr>
