			...
			<table>
				...
				<tbody class="alignC">
					...
					<tr<c:if test="${isNotice}"> style="background-color:#f9f0f9;"</c:if>>
						...
						<td class="title left">
							<a href="<c:out value="${URL_VIEW}"/>&${listIdxName}=${listKey}" title="상세보기" class="fn_btn_view" data-name="${listDt.NAME }" data-secret="${listDt.SECRET }"<c:if test="${listDt.RE_LEVEL > 1}"> style="padding-left:${(listDt.RE_LEVEL - 1) * 10}px;"</c:if><c:if test="${isDisplaySecretPassAuth}"> data-nm="<c:out value="${listKey}"/>"</c:if>>
								...
								<c:if test="${isListImg}"><span class="subject"><span></c:if>
									<c:out value="${listDt.SUBJECT}"/><strong class="point-color01">(<c:out value="${listDt.MEMO_COUNT}"/>)</strong>
						...
						<td>
							<c:if test="${listDt.REPLY_STATE == '0'}">미답변</c:if>
							<c:if test="${listDt.REPLY_STATE == '1'}">답변</c:if>
						</td>		  
						<td>
