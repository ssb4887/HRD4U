	...
	<div id="cms_board_article">
		...
		<h3><c:out value="${listDt.SUBJECT}"/></h3>
			<div class="one-box">
				<div class="half-box">
					<dl>
						<dt><spring:message code="item.reginame1.name"/></dt><dd><c:out value="${listDt.NAME}"/></dd>
			...
			<dl>
				...
				<dd>
					<%/* 답변글인 경우 */%>
					<c:if test="${(listDt.BRD_IDX != listDt.PNT_IDX and loginVO.usertypeIdx lt 40 and loginVO.memberNameOrg != listDt.NAME) || empty loginVO}">비공개</c:if>
					<%/* 질문글인 경우 */%>
					<c:if test="${listDt.BRD_IDX == listDt.PNT_IDX or loginVO.usertypeIdx ge 40 or loginVO.memberNameOrg == listDt.NAME}"><itui:objectView itemId="contents" objDt="${listDt}"/></c:if>
				</dd>
			</dl>
			</div>
			
		<c:set var="chkListDt" value="${listDt}"/>
		<%@ include file="../../common/viewBtnsQnaDoctor.jsp"%>
		</c:forEach>
		<c:if test="${settingInfo.use_memo eq '1'}">
		<%@ include file="../../common/viewMemoDoctor.jsp"%>
		</c:if>
