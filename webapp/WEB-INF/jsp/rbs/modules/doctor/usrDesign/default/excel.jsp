...
String titleName = "주치의 활동내역 목록현황_" + formattedDate + "";
	...
	<table summary="<%=titleName%> 목록을 제공합니다." style="width:100%;">
		...
		<thead>
			<tr>
				...
				<th scope="col">성과컨설팅</th>
				<th scope="col">훈련과정 맞춤개발</th>
		...
		<tbody class="alignC">
			...
			<c:forEach var="listDt" items="${list}" varStatus="i">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${listDt.DOCTOR_YN == 'Y'}">주치의</c:when>
					...
					<td>${listDt.DGNS}</td>
					<td>${listDt.CLINIC}</td>
					<td>${listDt.CONSULT}</td>
					<td>${listDt.DEVELOP}</td>
					<td>${listDt.EDU}</td>
				</tr>
			</c:forEach>
		</tbody>
