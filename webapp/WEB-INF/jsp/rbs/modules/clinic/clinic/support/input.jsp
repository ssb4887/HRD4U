...
$(function(){
	...
	$("#<c:out value="${param.inputFormId}"/> .fn_btn_submit").click(function(){
		try {
			...
			var varConfirm = confirm("능력개발클리닉 지원금 신청서를 " + statusName + "하시겠습니까?");
			if(!varConfirm) return false;
	...
	$("[id^='modal-action'] .fn-search-submit").on("click", function() {
    	...
    	$("input[id^='check0" + modalNum + "']:checked").each(function (){
    		...
    		innerHtml += '<span style="display:block;"><strong class="point-color01"><a href="${contextPath}/${crtSiteId}/clinic/activity_view_form.do?mId=68&acmsltIdx=' + acmsltIdx + '" target="_blank">' + itemName + ' 활동일지</a></strong></span>' +
	...
	$("[id^='reqstYn']").click(function() {
		...
		if(index == "1"){
			...
			if($(this).is(":checked")){
				var maxIdx = Number($("#maxIdx").val());
	    		$("#maxIdx").val(maxIdx + 1);
				var innerHtml = '<a href="${contextPath}/${crtSiteId}/clinic/plan_view_form.do?mId=67&planIdx=' + planIdx + '" target="_blank"><strong class="point-color01">훈련계획서</strong></a>' +
			...
			if(clinicResult != "0"){
				if($(this).is(":checked")){
					var maxIdx = Number($("#maxIdx").val());
		    		$("#maxIdx").val(maxIdx + 1);
					var innerHtml = '<a href="${contextPath}/${crtSiteId}/clinic/result_view_form.do?mId=87&resltIdx=' + resltIdx + '" target="_blank"><strong class="point-color01">성과보고서</strong></a>' +
