...
$(function(){
	...
	$("#<c:out value="${param.inputFormId}"/> .fn_btn_submit").click(function(){
		try {
			...
			var varConfirm = confirm("능력개발클리닉 성과보고서를 " + statusName + "하시겠습니까?");
			if(!varConfirm) return false;			
			return fn_resultInputFormSubmit(confmStatus);
		}catch(e){alert(e); return false;}
	});
	...
	$("[id^='reqstYn']").click(function() {
		alert("지원유형은 훈련계획서에서 신청한 항목과 동일하여 수정할 수 없습니다. 지원유형을 변경하기 위해서는 훈련계획서를 변경하고 승인받아야 합니다.");
		return false;
	});
...
function fn_resultInputFormSubmit(confmStatus) {
	// 신청할 때만 필수값 체크
	if(confmStatus == '5'){		
	...
	if(confmStatus != "5"){
		...
		// 에디터 ZWS 삭제
		$(".contents-area").find("textarea").each(function() {
