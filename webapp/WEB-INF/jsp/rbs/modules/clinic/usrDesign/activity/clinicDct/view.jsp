...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/activity/view.jsp"/>
	</jsp:include>
</c:if>
			...
			<div class="contents-box pl0">
				<div class="table-type02 horizontal-scroll">
					<table class="width-type02">
						<caption>능력개발클리닉 활동일지 정보표 : 기업명, 구분, 일시, 장소, 능력개발활동 내역, 증빙사진, 증빙내역, 활동지원금에 대한 정보표</caption>
						<colgroup>
							<col width="14%">
							<col width="auto">
							<col width="auto">
...
<!-- 								<td colspan="3">		 -->
<%-- 									<c:forEach var="listDt" items="${multiFileHashMap.photo}">	 --%>
<%-- 										<c:set var="fileName" value="${fn:split(listDt.FILE_SAVED_NAME, '.')}" /> --%>
<%-- 										<c:choose> --%>
<%-- 											<c:when test="${fileName[1] eq 'pdf'}"> --%>
<%-- 												<iframe width="100%" height="400" src="/clinicImg/${listDt.FILE_SAVED_NAME}"></iframe>	 --%>
<%-- 											</c:when> --%>
<%-- 											<c:otherwise> --%>
<%-- 												<img src="/clinicImg/${listDt.FILE_SAVED_NAME}" style="width:100%;"/> --%>
<%-- 											</c:otherwise> --%>
<%-- 										</c:choose> --%>
<%-- 									</c:forEach>		 --%>
<!-- 								</td>						 -->
...
<!-- 								<td colspan="3"> -->
<%-- 									<c:forEach var="listDt" items="${multiFileHashMap.bill}">	 --%>
<%-- 										<c:set var="fileName" value="${fn:split(listDt.FILE_SAVED_NAME, '.')}" /> --%>
<%-- 										<c:choose> --%>
<%-- 											<c:when test="${fileName[1] eq 'pdf'}"> --%>
<%-- 												<iframe width="100%" height="400" src="/clinicImg/${listDt.FILE_SAVED_NAME}" ></iframe>	 --%>
