...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/total/detailList.jsp"/>
					...
					<fieldset class="clear">
						...
						<div class="search-area">
							...
							<div class="search-box depth">
								...
								<button class="btn-m01 btn-color03" id="searchAll" value="searchAll" >전체</button>
							</div>
						</div>	
					</fieldset>
<!-- 				</form> -->
								...
								<td>
									...
									<c:choose>
										<c:when test="${listDt.GUBUNCD eq 'acmslt' || listDt.SPORT_ITEM_CD eq '07'}">
											<a href="${contextPath}/${crtSiteId}/clinicDct/activity_view_form.do?mId=68&acmsltIdx=${listDt.IDX}&bplNo=${corpInfo.BPL_NO}" class="btn-linked" style="display:inline;" target="_blank">활동 일지&nbsp;<img src="${contextPath}${imgPath}/icon/icon_search04.png" alt="활동일지 상세보기" style="display:inline;"></a>
										</c:when>
										<c:when test="${listDt.GUBUNCD eq 'plan' }">
											<a href="${contextPath}/${crtSiteId}/clinicDct/plan_view_form.do?mId=67&planIdx=${listDt.IDX}&bplNo=${corpInfo.BPL_NO}" class="btn-linked" style="display:inline;" target="_blank">훈련 계획서&nbsp;<img src="${contextPath}${imgPath}/icon/icon_search04.png" alt="훈련계획서 상세보기" style="display:inline;"></a>										
										</c:when>
										<c:when test="${listDt.GUBUNCD eq 'rslt' }">
											<a href="${contextPath}/${crtSiteId}/clinicDct/result_view_form.do?mId=87&resltIdx=${listDt.IDX}&bplNo=${corpInfo.BPL_NO}" class="btn-linked" style="display:inline;" target="_blank">성과 보고서&nbsp;<img src="${contextPath}${imgPath}/icon/icon_search04.png" alt="성과보고서 상세보기" style="display:inline;"></a>										
										</c:when>
										<c:when test="${listDt.GUBUNCD eq 'cnsl' }">
											<a href="<c:out value="${contextPath}/${crtSiteId}/consulting/viewAll.do?mId=126&cnslIdx=${listDt.IDX}&bplNo=${corpInfo.BPL_NO}"/>" class="btn-linked" style="display:inline;" target="_blank"><c:out value="${listDt.SPORT_ITEM_NM}"></c:out>&nbsp;<img src="${contextPath}${imgPath}/icon/icon_search04.png" alt="컨설팅 상세보기" style="display:inline;"></a>										
										</c:when>		
									</c:choose>
									</strong>
								</td>
