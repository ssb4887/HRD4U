...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		...
		<jsp:param name="javascript_page" value="${moduleJspRPath}/clinic/support/input.jsp"/>
	...
	<div class="contents-area">
			...
			<table class="width-type02">
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3" class="left">${year}년</td>
					</tr>
					<tr>
						<th scope="row" rowspan="3">지원금 지급계좌</th>
						<th scope="row" colspan="2">은행명&nbsp;<strong class="point-important">*</strong></th>
						<td colspan="4" class="left">
							<itui:objectSelect itemId="bankCd" itemInfo="${itemInfo}" objStyle="width:100%;"/>
							<input type="hidden" name="bankNm" id="bankNm" <c:if test="${submitType eq 'modify'}">value="${dt.BANK_NM}"</c:if>>
						</td>
					</tr>
					<tr>
						<th scope="row" colspan="2">예금주&nbsp;<strong class="point-important">*</strong></th>
						<td colspan="4" class="left"><itui:objectText itemId="dpstrNm" itemInfo="${itemInfo}" objClass="w100"/></td>
					</tr>
					<tr>
						<th scope="row" colspan="2">계좌번호&nbsp;<strong class="point-important">*</strong></th>
						<td colspan="4" class="left"><itui:objectText itemId="acnutNo" itemInfo="${itemInfo}" objClass="w100 onlyNum"/></td>
					</tr>
					<tr>
							...
							<td>
								...
								<c:choose>
									<c:when test="${submitType eq 'write'}">
										<c:if test="${i.count gt '2'}">
											<a href="#" class="btn-linked" id="open-modal${i.count}" data-idx="${i.count}" style="display:inline;"><img src="${contextPath}${imgPath}/icon/icon_search04.png" style="display:inline; margin-bottom:5px;"></a>
										</c:if>
										<span id="actUrl${i.count}"></span>
									</c:when>
									<c:otherwise>
										<c:choose>
											...
											<c:otherwise>
												<c:choose>
													...
													<c:when test="${listDt.SPORT_CD eq '09'}"><c:set var="itemName" value="HRD 성과교류"/></c:when>
												</c:choose>
					...
					<tr>
						<th scope="row" colspan="6">신청 금액 합계(원)</th>
						<td>
		...
		<div class="btn-area">
			<div class="btns-right">
				...
				<button type="button" class="btn-m01 btn-color01 depth3 fn_btn_cancel" onclick="return confirm('목록으로 이동하시겠습니까? \n임시저장을 하지 않으면 작성한 내용은 저장되지 않습니다.')">목록</button>
			</div>
		</div>
		</form>
	</div>
			...
			<div class="contents-box pl0">
				...
				<div class="contents-box pl0">
					<div class="table-type01 horizontal-scroll">
						...
						<thead>
						<tr>										
							<th scope="col"><input type="checkbox" id="selectAll" name="selectAll" data-code="03" class="checkbox-type01" title="<spring:message code="item.select.all"/>"/></th>
						...
						<tbody class="alignC">
							...
							<tr>
								<td><input type="checkbox" name="select" value="${act04.ACMSLT_IDX}" data-code="03" class="checkbox-type01" id="check03${act04.ACMSLT_IDX}" data-pay="${act04.ACMSLT_SPORT_AMT}" data-acexp="${act04.ACEXP_SPORT_YN}"/></td>
								<td><fmt:formatDate pattern="yyyy-MM-dd" value="${act04.ACMSLT_START_DT}"/></td>
								<td>
									<a href="${contextPath}/${crtSiteId}/clinic/activity_view_form.do?mId=68&acmsltIdx=${act04.ACMSLT_IDX}" target="_blank" class="btn-linked" style="display:inline;">
				...
				<div class="contents-box pl0">
					<div class="table-type01 horizontal-scroll">
						<table class="width-type02">
							...
							<thead>
							<tr>										
								<th scope="col"><input type="checkbox" id="selectAll" name="selectAll" data-code="04" class="checkbox-type01" title="<spring:message code="item.select.all"/>"/></th>
								<th scope="col">활동시작일</th>
								<th scope="col">제목</th>
								<th scope="col">등록일</th>
							</tr>
							...
							<tbody class="alignC">
								...
								<tr>
									<td><input type="checkbox" name="select" value="${act08.ACMSLT_IDX}" data-code="04" class="checkbox-type01" id="check04${act08.ACMSLT_IDX}" data-pay="${act08.ACMSLT_SPORT_AMT}" data-acexp="${act08.ACEXP_SPORT_YN}"/></td>
									<td><fmt:formatDate pattern="yyyy-MM-dd" value="${act08.ACMSLT_START_DT}"/></td>
									<td>
										<a href="${contextPath}/${crtSiteId}/clinic/activity_view_form.do?mId=68&acmsltIdx=${act08.ACMSLT_IDX}" target="_blank" class="btn-linked" style="display:inline;">
				...
				<div class="contents-box pl0">
					<div class="table-type01 horizontal-scroll">
						<table class="width-type02">
							...
							<thead>
							<tr>										
								<th scope="col"><input type="checkbox" id="selectAll" name="selectAll" data-code="05" class="checkbox-type01" title="<spring:message code="item.select.all"/>"/></th>
								<th scope="col">활동시작일</th>
								<th scope="col">제목</th>
								<th scope="col">등록일</th>
							</tr>
							...
							<tbody class="alignC">
								...
								<tr>
									<td><input type="checkbox" name="select" value="${act09.ACMSLT_IDX}" data-code="05" class="checkbox-type01" id="check05${act09.ACMSLT_IDX}" data-pay="${act09.ACMSLT_SPORT_AMT}" data-acexp="${act09.ACEXP_SPORT_YN}"/></td>
									<td><fmt:formatDate pattern="yyyy-MM-dd" value="${act09.ACMSLT_START_DT}"/></td>
									<td>
										<a href="${contextPath}/${crtSiteId}/clinic/activity_view_form.do?mId=68&acmsltIdx=${act09.ACMSLT_IDX}" target="_blank" class="btn-linked" style="display:inline;">
