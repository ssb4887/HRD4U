...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/clinic/result/view.jsp"/>
	</jsp:include>
</c:if>
...
<div class="contents-wrapper">
	<div class="contents-area">
		<h3 class="title-type01 ml0">능력개발클리닉 성과보서</h3>
		 ...
		 <div class="contents-box pl0">
			<div class="table-type02 horizontal-scroll">
				 <table class="width-type02">
		            ...
		            <thead>
			            <tr>
					    	<th scope="col" colspan="3">훈련실시 개요</th>
					    </tr>	
		            </thead>
		            <tbody>
					    <tr>
		...
		<div class="contents-box pl0">
	 	...
	 	<c:if test="${dt.CONFM_STATUS gt '10' && !empty dt.DOCTOR_OPINION}">	
	 		<div class="contents-box pl0">
	 			<div class="table-type02 horizontal-scroll">
					<table class="width-type02">
						<caption>능력개발클리닉 성과보고서 중 주치의 의견 정보표</caption>
						<colgroup>
							<col width="15%">
							<col width="auto">
						</colgroup>
			...
			<div class="btns-right">
				...
				<c:choose>
					<c:when test="${dt.CONFM_STATUS eq '5' || dt.CONFM_STATUS eq '20' || dt.CONFM_STATUS eq '40'}">
						<a href="<c:out value="${RESULT_MODIFY_FORM_URL}"/>&resltIdx=${dt.RESLT_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">수정</a>
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '10'}">
						<a href="<c:out value="${RESULT_WITHDRAW_URL}"/>&resltIdx=${dt.RESLT_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">회수</a>						
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '30'}">
						<a href="<c:out value="${RESULT_RETURNREQUEST_URL}"/>&resltIdx=${dt.RESLT_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">반려요청</a>
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '55'}">
						<a href="<c:out value="${RESULT_DROPREQUEST_URL}"/>&resltIdx=${dt.RESLT_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">중도 포기</a>
					</c:when>
				</c:choose>
