...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		...
		<jsp:param name="javascript_page" value="${moduleJspRPath}/plan/input.jsp"/>
...
<div class="contents-wrapper">
	<div class="contents-area">
		<form id="${inputFormId}" name="${inputFormId}" method="post" action="<c:out value="${PLAN_MODIFY_URL}"/>" target="submit_target" enctype="multipart/form-data">
		<h3 class="title-type01 ml0">능력개발클리닉 훈련계획서</h3>
		<input type="hidden" id="planIdx" name="planIdx" value="${dt.PLAN_IDX}" />
		<input type="hidden" id="cliIdx" name="cliIdx" value="${dt.CLI_IDX}" />
		...
		<div class="btns-area">
			...
			<div class="btns-right">
				...
				<button type="submit" title="저장하기" class="btn-m01 btn-color03 depth3 fn_btn_submit">저장</button>
				<a href="<c:out value="${PLAN_LIST_FORM_URL}"/>" title="목록으로 이동" class="btn-m01 btn-color01 depth3 fn_btn_write" onclick="return confirm('목록으로 이동하시겠습니까? \n저장을 하지 않으면 수정한 내용은 저장되지 않습니다.')">목록</a>
			</div>
		</div>
		</form>
	</div>
