...
<c:if test="${!empty TOP_PAGE}">
	<jsp:include page="${TOP_PAGE}" flush="false">
		<jsp:param name="javascript_page" value="${moduleJspRPath}/clinic/plan/view.jsp"/>
	</jsp:include>
</c:if>
			...
			<div class="btns-right">
				...
				<c:choose>
					<c:when test="${dt.CONFM_STATUS eq '5' || dt.CONFM_STATUS eq '20' || dt.CONFM_STATUS eq '40'}">
						<a href="<c:out value="${PLAN_MODIFY_FORM_URL}"/>&planIdx=${dt.PLAN_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">수정</a>
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '10'}">
						<a href="<c:out value="${PLAN_WITHDRAW_URL}"/>&planIdx=${dt.PLAN_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color08 depth3 fn_btn_modify">회수</a>						
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '30'}">
						<a href="<c:out value="${PLAN_RETURNPLAN_URL}"/>&planIdx=${dt.PLAN_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">반려요청</a>
					</c:when>
					<c:when test="${dt.CONFM_STATUS eq '55' || dt.CONFM_STATUS eq '53'}">
						<a href="<c:out value="${PLAN_MODIFY_FORM_URL}"/>&planIdx=${dt.PLAN_IDX}" class="btn-m01 btn-color03 depth3 fn_btn_modify">수정</a>
						<a href="<c:out value="${PLAN_DROPREQUEST_URL}"/>&planIdx=${dt.PLAN_IDX}&cliIdx=${dt.CLI_IDX}" class="btn-m01 btn-color08 depth3 fn_btn_modify">중도 포기</a>
					</c:when>
				</c:choose>
