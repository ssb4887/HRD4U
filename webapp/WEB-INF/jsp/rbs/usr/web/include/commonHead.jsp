	...
	<title>
		<c:out value="${siteInfo.site_title}"/>-<c:out value="${crtMenu.menu_name}"/>
		<c:choose>
			<c:when test="${crtMenu.module_id eq 'board' && submitType eq 'list'}">(목록)</c:when>
			<c:when test="${crtMenu.module_id eq 'board' && submitType eq 'view'}">(상세)</c:when>
			<c:when test="${crtMenu.module_id eq 'board' && (submitType eq 'write' || submitType eq 'modify')}">(등록)</c:when>
			<c:otherwise></c:otherwise>
		</c:choose>
	</title>
	<meta name="subject" content="HRD4U 기업직업훈련 지원시스템" />
	<meta name="author" content="HRD4U 기업직업훈련 지원시스템" />
	<meta name="keywords" content="HRD4U 기업직업훈련 지원시스템, 기업훈련지원, HRD기초진단, 기업HRD이음컨설팅, 훈련패키지" />
