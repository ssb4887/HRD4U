...
<c:set var="defaultListUrl" value="${listUrl}&${pageName}="/>
	...
	<a href="<c:out value="${defaultListUrl}${firstPageNo}"/>#tab3" class="btn-first" title="처음 페이지"></a>
	<a href="<c:out value="${defaultListUrl}${currentPageNo-1}"/>#tab3" class="btn-preview" title="이전 페이지"></a>
	</c:if>
	<!-- 반복구간 -->
	<c:forEach var="pageNo" begin="${firstPageNoOnPageList}" end="${lastPageNoOnPageList}">
		<c:choose>
			...
			<c:otherwise><a href="<c:out value="${defaultListUrl}${pageNo}"/>#tab3" title="${pageNo} 페이지">${pageNo}</a></c:otherwise>
		</c:choose>
	</c:forEach>
	<!-- 반복구간 -->
	<c:if test="${totalPageCount > currentPageNo}">
	<a href="<c:out value="${defaultListUrl}${currentPageNo+1}"/>#tab3" class="btn-next" title="다음 페이지"></a>
	<a href="<c:out value="${defaultListUrl}${lastPageNo}"/>#tab3" class="btn-last" title="끝 페이지"></a>
	</c:if>
	</p>
<!-- 페이지네이션 구간 -->
