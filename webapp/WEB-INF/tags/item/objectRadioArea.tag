...
<c:forEach var="optnDt" items="${multiDataHashMap[itemId]}" varStatus="i">
	<c:if test="${empty optnDt.ISHIDDEN || optnDt.ISHIDDEN == '0' || optnDt.OPTION_CODE == objVal}">
	<div class="input-checkbox-area" id="div${id}${optnDt.INSTT_IDX}">
	<label for="total${id}${optnDt.INSTT_IDX}">
		<input type="radio" id="total${id}${optnDt.INSTT_IDX}" name="total${name}" value="<c:out value="${optnDt.INSTT_IDX}"/>"<c:if test="${optnDt.SMRIZE_INSTT_YN == 'Y'}"> checked="checked"</c:if><c:if test="${!empty objAttr}"> ${objAttr}</c:if>/> <c:out value="${optnDt.INSTT_NAME}"/>
