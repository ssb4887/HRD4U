	...
	<c:set var="isUseMoreItem" value="false"/>
			...
			<c:if test="${!empty itemListSearch}">
				...
				<c:if test="${!empty itemSearchOrder}">
					...
					<c:forEach var="itemSearchId" items="${itemSearchOrder}" varStatus="i">
						<c:if test="${itemSearchId ne keyFieldItemName}">
						...
						<c:if test="${(i.count % 2) == 1 }"><div class="one-box"></c:if>
						<c:if test="${itemKeyId ne 'is_month'}"><div class="half-box"></c:if>
						<dl class="dl_${itemKeyId}">
						...
						<dd class="dd_${itemKeyId}">
						<c:if test="${namePt == 1}">${itemKeyName}</c:if>
						<c:choose>
							<c:when test="${formatType == 100}">
							<% /* 시작일 종료일 달력으로 선택하기 */ %>
							<c:set var="dateType" value="${keyItem.date_type}"/>
							...
							<%-- <input type="text" id="${itemKeyId}" name="${itemKeyId}" title="${titleStart}" class="${objClass}" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${queryString[itemKeyId]}"/>"/>
								<span><button type="button" id="fn_btn_${itemKeyId}" title="${titleStart}" class="${btnClass}" data-idName="${itemKeyId}"><c:out value="${itemKeyName}"/></button></span> --%>
							<c:set var="titleStart" value="${objTitle}시작일"/>
							<c:set var="titleEnd" value="${objTitle}종료일"/>
							...
							<div class="input-calendar-area">		
								<input type="text" id="${itemKeyId}1" name="${itemKeyId}1" title="${titleStart}" class="sdate" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${queryString[value1Name]}"/>"/>
								<span><button type="button" id="fn_btn_${itemKeyId}1" title="${titleStart}" class="${btnClass}" data-idName="${itemKeyId}1" style="float:left; padding:0 10px; height:0px; margin-left:0;"></button></span>
							</div>
							<c:out value="~"/>
							<div class="input-calendar-area"  style="display:flex; margin-left:5px;">		
								<input type="text" id="${itemKeyId}2" name="${itemKeyId}2" title="${titleEnd}" class="edate" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${queryString[value2Name]}"/>"/>
								<span><button type="button" id="fn_btn_${itemKeyId}2" title="${titleEnd}" class="${btnClass}" data-idName="${itemKeyId}2" style="float:left; padding:0 10px; height:0px; margin-left:0;"></button></span>
							</div>
							</div>
							</c:when>
							...
							<c:choose>
								<c:when test="${!empty keyItem.option_type && keyItem.option_type != 0}"><c:set var="optnOptList" value="${searchOptnHashMap[itemSearchId]}"/></c:when>
								<c:when test="${!empty authList}"><c:set var="optnOptList" value="${authList}"/></c:when>
								<c:when test="${!empty insttList}"><c:set var="optnOptList" value="${insttList}"/></c:when>
								<c:otherwise><c:set var="optnOptList" value="${searchOptnHashMap[keyItem['master_code']]}"/></c:otherwise>
							</c:choose>
							<select name="${itemKeyId}" id="${itemKeyId}" class="select" title="${itemKeyName}"<c:if test="${!empty keyItem.style_width}"> style="width:${keyItem.style_width}%;"</c:if>>
							...
							<% /* 3차 select */ %>
								...
								<c:forEach var="levelIdx" begin="1" end="${optMaxLevel}">
									...
									<select id="${itemKeyId}${levelIdx}" name="${itemKeyId}_tmp" title="${itemTitle}" style="${objStyle}" class="select t_select <c:if test="${optMaxLevel eq 2}"> w50</c:if>"<c:if test="${required == 1}"> required="required"</c:if>>
										<c:choose>
											<c:when test="${keyItem['master_code'] eq 'SIDO_SIGUNGU'}">
												<c:choose>
													<c:when test="${levelIdx eq 1}"><c:set var="optTitle" value="시도"/></c:when>
													<c:otherwise><c:set var="optTitle" value="시군구"/></c:otherwise>
												</c:choose>
												<option value=""><c:out value="${optTitle}"/> 선택</option>
											</c:when>
											<c:otherwise>
							...
							<c:otherwise>
								<input type="text" name="${itemKeyId}" id="${itemKeyId}" title="${itemKeyName}" placeholder="${itemKeyName}" <c:if test="${!empty keyItem.style_width}"> style="width:${keyItem.style_width}px;"</c:if> value="<c:out value="${queryString[itemKeyId]}"/>"/>
							</c:otherwise>
						</c:choose>
						...
						<c:if test="${itemKeyId ne 'is_month'}"></div></c:if>
						<c:if test="${(i.count % 2) == 0 }"></div></c:if>
								</c:if>
								</c:if>
							</c:if>
					</c:forEach>
