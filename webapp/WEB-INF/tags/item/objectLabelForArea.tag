...
<label for="${id}"<c:if test="${!empty objStyle}"> style="${objStyle}"</c:if><c:if test="${!empty objClass}"> class="${objClass}"</c:if>><span>${preStr}<itui:objectItemName itemId="${itemId}" itemInfo="${itemInfo}" removeItemName="${removeItemName}" itemName="${itemName}" itemNameCRStr="${itemNameCRStr}"/><c:if test="${!isAdmMode && required == 1}"><strong class="point-important">*</strong></c:if>${nextStr}</span></label></c:if>