...
<c:forEach var="optnDt" items="${optnHashMap[itemObj['master_code']]}" varStatus="i">
	...
	<label for="${id}${i.count}" class="mr20">
		<input type="radio" id="${id}${i.count}" name="${name}" class="radio-type01" value="<c:out value="${optnDt.OPTION_CODE}"/>"<c:if test="${optnDt.OPTION_CODE == objVal || empty objVal && optnDt.OPTION_CODE == defVal}"> checked="checked"</c:if><c:if test="${optnDt.OPTION_CODE == defVal}"> data-default="1"</c:if><c:if test="${!empty objAttr}"> ${objAttr}</c:if>/> <c:out value="${optnDt.OPTION_NAME}"/>
