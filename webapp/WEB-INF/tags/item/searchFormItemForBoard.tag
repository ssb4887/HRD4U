		...
		<c:if test="${!empty itemSearchOrder}">
			...
			<form name="${formName}" id="${formId}" method="get"  action="<c:out value="${formAction}"/>">
				...
				<fieldset>
					...
					<itui:searchFormItemInForBoard itemListSearch="${itemListSearch}" searchOptnHashMap="${searchOptnHashMap}" isUseRadio="${isUseRadio}" isUseMoreItem="${isUseMoreItem}"/>
					<jsp:doBody/>
						...
						<button type="submit" id="${submitBtnId}" class="${submitBtnClass} btn-search" value="${submitBtnValue}" title="${submitBtnValue}">검색</button>
