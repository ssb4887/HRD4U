...
<th scope="row"<c:if test="${thColspan > 0}"> colspan="${thColspan}"</c:if>><itui:objectLabel itemId="${itemId}" nextItemId="1" required="${required}" itemInfo="${itemInfo}"/></th>

	<itui:objectCheckbox2 itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" defVal="${defVal}" objVal="${objVal}" optnHashMap="${optnHashMap}" objOptionStyle="${objOptionStyle}"/>
