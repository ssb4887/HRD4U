...
<c:if test="${objVal == null}"><c:set var="objVal" value="${objDt[itemColumnId]}"/></c:if>
	...
	<itui:objectViewArea itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" defVal="${defVal}" objVal="${objVal}" optnHashMap="${optnHashMap}" downloadUrl="${downloadUrl}" classSplitStr="${classSplitStr}"/>
	</dd>
</c:if>