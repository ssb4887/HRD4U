...
<c:forEach var="optnDt" items="${optnHashMap[itemObj['master_code']]}" varStatus="i">
	...
	<div class="input-checkbox-area">
		...
		<input type="radio" id="${itemId}${i.count}" name="${name}" value="<c:out value="${optnDt.OPTION_CODE}"/>"<c:if test="${optnDt.OPTION_CODE == objVal || empty objVal && optnDt.OPTION_CODE == defVal}"> checked="checked"</c:if><c:if test="${optnDt.OPTION_CODE == defVal}"> data-default="1"</c:if><c:if test="${!empty objAttr}"> ${objAttr}</c:if>/>
	...
	<label for="${itemId}${i.count}"><c:out value="${optnDt.OPTION_NAME}"/></label>
	</div>
	</c:if>
</c:forEach>
