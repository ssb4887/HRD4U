...
<c:forEach var="optnDt" items="${optnHashMap[itemObj['master_code']]}" varStatus="i">
	...
	<c:set var="objValDt" value="${elfn:getMatchHashMap(objValList, optnDt.OPTION_CODE, 'INSTT_IDX')}"/>
	...
	<div class="input-checkbox-area">
		<label for="${itemId}${i.count}">
			...
			<input type="checkbox" id="${itemId}${i.count}" name="${itemId}" class="checkbox-type01" value="<c:out value="${optnDt.OPTION_CODE}"/>"<c:if test="${objChked}"> checked="checked"</c:if> data-regi="<c:out value="${optnDt.OPTION_NAME}"/>"/> <c:out value="${optnDt.OPTION_NAME}"/>
