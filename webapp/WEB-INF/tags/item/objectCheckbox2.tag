...
<c:forEach var="optnDt" items="${optnHashMap[itemObj['master_code']]}" varStatus="i">
	...
	<c:choose>
		<c:when test="${i.count == 1}"><c:set var="itemList" value="menu"/><c:set var="chkVal" value="menu"/></c:when>
		<c:when test="${i.count == 2}"><c:set var="itemList" value="create"/><c:set var="chkVal" value="menu"/></c:when>
		<c:when test="${i.count == 3}"><c:set var="itemList" value="read"/><c:set var="chkVal" value="menu"/></c:when>
		<c:when test="${i.count == 4}"><c:set var="itemList" value="modify"/><c:set var="chkVal" value="menu"/></c:when>
		<c:when test="${i.count == 5}"><c:set var="itemList" value="del"/><c:set var="chkVal" value="menu"/></c:when>
		<c:when test="${i.count == 6}"><c:set var="itemList" value="file"/><c:set var="chkVal" value="menu"/></c:when>
	</c:choose>
	<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<label for="${itemList}${name }">
		<c:set var="objChked" value="${!empty objValDt || empty objValList && elfn:arrayIndexOf(defVals, optnDt.OPTION_CODE) != -1}"/>
		<input type="checkbox" id="${itemList}${name }" name="${name }" value="${chkVal }"<c:if test="${objChked}"> checked="checked"</c:if>/> <c:out value="${optnDt.OPTION_NAME}"/>
