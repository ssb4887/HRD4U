...
<c:choose>
	...
	<c:otherwise>
	<dt <c:if test="${thColspan > 0}"> colspan="${thColspan}"</c:if> <c:if test="${rowspan > 0}">rowspan="${rowspan}"</c:if><c:if test="${!empty thClass}"> class="${thClass}"</c:if>><itui:objectLabelForArea itemId="${itemId}" id="${id}" required="${required}" itemInfo="${itemInfo}" itemName="${itemName2}" itemNameCRStr="${itemNameCRStr}"/></dt>
	<dd<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
		<itui:objectTextForArea itemId="${itemId}" id="${id}" name="${name}" itemInfo="${itemInfo}" itemLangList="${itemLangList}" objDt="${objDt}" objVal="${objVal}" objClass="${objClass}" objStyle="${objStyle}" objAttr="${objAttr}"/>
