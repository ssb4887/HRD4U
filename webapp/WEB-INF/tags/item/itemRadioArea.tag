...
<dt<c:if test="${thColspan > 0}"> colspan="${thColspan}"</c:if><c:if test="${!empty thClass}"> class="${thClass}"</c:if>><itui:objectLabelForArea itemId="${itemId}" id="${id}1" required="${required}" itemInfo="${itemInfo}"/></dt>
<dd id="regi">
	<div class="input-checkbox-wrapper ratio">
		<itui:objectRadioArea itemId="${itemId}" id="${id}" name="${name}" itemInfo="${itemInfo}" objDt="${objDt}" defVal="${defVal}" objVal="${objVal}" optnHashMap="${optnHashMap}" objAttr="${objAttr}" objOptionStyle="${objOptionStyle}"/>
	</div>
</dd>
</c:if>