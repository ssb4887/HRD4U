		...
		<c:choose>
			...
			<c:when test="${empty itemFormatType || itemFormatType == 0}">
				...
				<c:when test="${itemObjectType == 5}">
					<%/* radio */%>
					<itui:objectRadioBoard itemId="${itemId}" itemInfo="${itemInfo}" optnHashMap="${optnHashMap}"/>
				</c:when>
				...
				<c:when test="${itemObjectType == 9}">
					<%/* multi file */%>
					<itui:objectMultiFileCustom itemId="${itemId}" itemInfo="${itemInfo}" objStyle="${itemObjStyle}" downLink="${isAdmMode}" inputTypeName="${inputFlag}"/>
				</c:when>
