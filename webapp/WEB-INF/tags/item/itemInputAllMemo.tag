...
<div class="comment-form-area">
	<div class="comment-body-wrapper">
		<itui:itemInputMemo inputTypeName="${inputTypeItemName}" itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" optnHashMap="${optnHashMap}" multiFileHashMap="${multiFileHashMap}" colspan="${colspan}" itemLangList="${itemLangList}"/>
		<button type="submit" class="btn-comment-save" value="저장" title="저장">저장</button>
	</div>
</div>
			</c:if>
			</c:if>
