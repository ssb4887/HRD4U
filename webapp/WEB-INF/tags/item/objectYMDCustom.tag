...
<c:if test="${!empty itemObj}">
	...
	<c:choose>
		<c:when test="${inputTypeName eq 'start'}">
			<c:set var="date" value="sdate" />
			<c:set var="placeholder" value="시작일" />
		</c:when>
		<c:when test="${inputTypeName eq 'end'}">
			<c:set var="date" value="edate" />
			<c:set var="placeholder" value="종료일" />
		</c:when>
	</c:choose>
	
	<c:if test="${itemObj['date_type'] == 1}"><fmt:formatDate var="objVal" value="${objVal}" pattern="yyyy-MM-dd"/></c:if>
	<div class="input-calendar-area" style="${objStyle}">		
		<input type="text" id="${id}" name="${itemId}" title="${itemName}" class="${date}" <c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${objVal}"/>" placeholder="${placeholder}"/>
<%-- 		<span><button type="button" id="${btnId}" title="${btnText}" class="ui-datepicker-trigger"><img src="../../assets/img/board/icon_calendar01.gif" alt="${btnText}" style="margin-top:-1px;"></button></span> --%>
	</div>
</c:if>