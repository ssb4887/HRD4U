	...
	<c:choose>
		...
		<c:when test="${itemObjectType == 3 || itemObjectType == 4 || itemObjectType == 11}">
			...
			<c:choose>
					...
					<ul class="fn_mult">
						<li>
							<c:forEach var="objValDt" items="${multiList}" varStatus="i">
								<c:if test="${i.first }"><c:out value="${objValDt.INSTT_NAME}"/></c:if>
								<c:if test="${!i.first }">, <c:out value="${objValDt.INSTT_NAME}"/></c:if>
							</c:forEach>
						</li>
					</ul>
				...
				<c:otherwise>
					<p>${multiList }////${defVal}1231321${multiDataHashMap}sdasd</p>
				</c:otherwise>
			</c:choose>
		</c:when>
		<c:when test="${itemObjectType == 22}">
		...
		<%/* multi file */%>
			...
			<c:forEach var="optnDt" items="${fileList}" varStatus="i">
				...
				<c:choose>
					...
					<c:otherwise>
						<p class="attached-file"><a href="<c:out value="${URL_DOWNLOAD}"/>&${keyItemId}=<c:out value="${optnDt[keyColumnId]}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${optnDt.ITEM_ID}"/>" class="fn_filedown"><c:out value="${optnDt[originColumnId]}"/><c:if test="${!empty optnDt.FILE_TEXT }"> : <c:out value="${optnDt.FILE_TEXT}"/></c:if></a></p>
					</c:otherwise>
				</c:choose>
				</li>
			</c:forEach>
