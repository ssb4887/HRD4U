	...
	<c:forEach var="itemId" items="${itemOrder}">
		<c:if test="${elfn:arrayIndexOf(exceptIds, itemId) == -1}">
			...
			<c:if test="${!empty itemObj}">
				...
				<div class="one-box">
					<dl>
						<itui:itemInputForDct inputTypeName="${inputTypeItemName}" itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" optnHashMap="${optnHashMap}" multiFileHashMap="${multiFileHashMap}" colspan="${colspan}" itemLangList="${itemLangList}"/>
					</dl>
				</div>
			</c:if>
			</c:if>
