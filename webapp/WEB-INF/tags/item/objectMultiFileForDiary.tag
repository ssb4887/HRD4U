...
<c:if test="${!empty itemObj}">
	...
	<ul class="width100 ${ulClass}">
		<c:if test="${crtSiteId  == 'dct' }">
		<li class="fileBox">
			<input type="text" class="fileName" placeholder="회의사진 첨부" title="회의사진 첨부" readonly>
		...
		<li class="fn_multiFileDown" style="text-align:left; padding:10px 0 0 0;">
			<dl class="saved_file_nb">
				<dt><c:if test="${crtSiteId  == 'dct' }"><spring:message code="item.file.saved.name"/></c:if></dt>
				<dd>
				<ul>
				<c:forEach var="optnDt" items="${fileList}" varStatus="i">
					<li><a href="diaryDownload.do?mId=133&${keyItemId}=<c:out value="${optnDt[keyColumnId]}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${itemId}"/>"><img src="../images/common/ico_file.gif" alt="파일"><c:out value="${optnDt.FILE_ORIGIN_NAME}"/></a></li>
				</c:forEach>
				</ul>
				</dd>
			</dl>
