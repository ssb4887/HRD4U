...
<c:if test="${!empty itemObj}">
	...
	<textarea id="${id}" name="${name}" title="${itemName}" class="${objClass}" style="${objStyle}" data-type="<c:out value="${contentsType}"/>"<c:if test="${!useEditor && required == 1}"> </c:if> ${objAttr}><c:out value="${objVal}"/></textarea>
