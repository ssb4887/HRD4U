...
<c:if test="${objVal == null}"><c:set var="objVal" value="${objDt[itemColumnId]}"/></c:if>
	<dl>
		...
		<dd>
			<itui:objectViewCustom itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" defVal="${defVal}" objVal="${objVal}" optnHashMap="${optnHashMap}" downloadUrl="${downloadUrl}" classSplitStr="${classSplitStr}" homepageUrl="${homepageUrl}" contextPath="${contextPath}" imgPath="${imgPath}"/>
		</dd>
	</dl>
</c:if>