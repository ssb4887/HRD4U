			...
			<form name="${formName}" id="${formId}" method="get"  action="<c:out value="${formAction}"/>">
				...
				<fieldset>
					...
					<div class="basic-search-wrapper">
						<div class="one-box">
							<itui:searchFormItemInForDct3 itemListSearch="${itemListSearch}" searchOptnHashMap="${searchOptnHashMap}" isUseRadio="${isUseRadio}" isUseMoreItem="${isUseMoreItem}"/>
						</div>
						<div class="one-box">
							<c:forEach var="itemSearchId" items="${itemSearchOrder}">
								<c:if test="${itemSearchId == keyFieldItemName}">
									...
									<!-- 옵션 -->
									<dl>
										<dt id="fn_dt_${keyFieldInfo.select_name}" style="padding:0 0 0 15px;"><label for="${keyFieldInfo.select_name}">검색</label></dt>
										<dd>
											<div class="input-search-wrapper" style="display:flex;">
											<select name="${keyFieldInfo.select_name}" id="${keyFieldInfo.select_name}" class="select" title="검색옵션" style="width:20%;">
											...
											<input type="text" name="${keyFieldInfo.query_name}" id="${keyFieldInfo.query_name}" value="<c:out value="${queryString[keyFieldInfo.query_name]}"/>" title="검색어" class="inputSearch" style="width:76% !important; margin: 0 0 0 4% !important; border:0px;" />
											</div>
										</dd>
									</dl>
									</c:if>
					...
					<div class="btns-area" style="display:block; text-align:center;">
