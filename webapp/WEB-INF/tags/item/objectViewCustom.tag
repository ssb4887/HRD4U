...
<c:if test="${!empty corpInfo}"><c:set var="corpInfo" value="${corpInfo}"/></c:if>
<c:choose>
	...
	<c:choose>
		...
		<%/* multi file */%>
			...
			<c:forEach var="optnDt" items="${fileList}" varStatus="i">
				...
				<c:choose>
					<c:when test="${!empty downloadUrl}">
						<p class="attached-file"><a href="<c:out value="${downloadUrl}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${optnDt.ITEM_ID}"/>" class="fn_filedown"><c:out value="${optnDt[originColumnId]}"/></a></p>
					</c:when>
					<c:otherwise>
						<p class="attached-file"><a href="<c:out value="${URL_DOWNLOAD}"/>&${keyItemId}=<c:out value="${optnDt[keyColumnId]}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${optnDt.ITEM_ID}"/>" class="fn_filedown"><c:out value="${optnDt[originColumnId]}"/></a></p>
					</c:otherwise>
				</c:choose>
				</li>
			</c:forEach>
		...
		<c:otherwise>
			<c:choose>
				<c:when test="${homepageUrl eq 'yes'}">
					<p class="lineked-homepage">
						<c:if test="${!empty objVal}">
							<strong><c:out value="${objVal}"/></strong>
							<a href="<c:out value="${objVal}"/>" class="btn-linked" target="_blank" title="새창 열림"><img src="${contextPath}${imgPath}/icon/icon_search04.png" /></a>
						</c:if>
					</p>
				</c:when>
				<c:when test="${!empty corpInfo && !empty itemObj['is_exist']}">
					<c:if test="${corpInfo[itemColumnId] ne objVal}"><c:set var="objClass" value="point-color04"/></c:if>
						<span class="${objClass}"><c:out value="${objVal}"/></span>
				</c:when>
				<c:otherwise>
					<c:if test="${(itemId eq 'applyInduty' || itemId eq 'fthtecCate') && empty objVal}">-</c:if>
					<c:out value="${objVal}"/><c:if test="${itemId eq 'trDaycnt'}">일</c:if>
					<c:out value="${itemObj['next_text']}"/>
				</c:otherwise>
			</c:choose>
		</c:otherwise>
