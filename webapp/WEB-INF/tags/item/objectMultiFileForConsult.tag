...
<c:if test="${!empty itemObj}">
	...
	<ul class="width100 ${ulClass}">
		<c:if test="${crtSiteId  == 'web' }">
		<li class="fileBox">
			<input type="text" class="fileName" placeholder="첨부 파일" title="첨부 파일" readonly>
		...
		<li class="input-add-btns" style="margin-top: 10px;">
			...
			<button type="button" id="fn_btn_text_${itemId}"><spring:message code="item.file.text.input.name"/></button>
		</li>
		<li>
			<c:choose>
				<c:when test="${!empty minimum && minimum > 0}">
		...
		<li class="fn_multiFileDown" style="text-align:left; padding:10px 0 0 0;">
			<dl class="saved_file_nb">
				...
				<ul>
				<c:forEach var="optnDt" items="${fileList}" varStatus="i">
					<li><a href="<c:out value="${URL_DOWNLOAD}"/>&${keyItemId}=<c:out value="${optnDt[keyColumnId]}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${itemId}"/>"><img src="../images/common/ico_file.gif" alt="파일"><c:out value="${optnDt.FILE_ORIGIN_NAME}"/></a></li>
				</c:forEach>
				</ul>
				</dd>
			</dl>
