...
<c:if test="${!empty itemObj}">
	...
	<c:set var="masterCode" value="${itemObj['master_code']}"/>
	<c:choose>
		...
		<c:otherwise>
			...
			<c:forEach var="levelIdx" begin="1" end="${optMaxLevel}">
				...
				<select id="${id}${levelIdx}" name="${name}_tmp" title="${itemTitle}" style="${objStyle}" class="select t_select ${objClass}"<c:if test="${required == 1}"> required="required"</c:if>>
					<c:choose>
						<c:when test="${masterCode eq 'NCS_CODE'}">
							<c:choose>
								<c:when test="${levelIdx eq 1}"><c:set var="optTitle" value="대"/></c:when>
								<c:when test="${levelIdx eq 2}"><c:set var="optTitle" value="중"/></c:when>
								<c:otherwise><c:set var="optTitle" value="소"/></c:otherwise>
							</c:choose>
							<option value="">NCS <c:out value="${optTitle}"/>분류</option>
						</c:when>
						<c:when test="${masterCode eq 'SIDO_SIGUNGU'}">
							<c:choose>
			...
			<select id="${id}" name="${name}" title="${itemName}" style="display:none;" class="select t_select" data-max="${optMaxLevel}">
				<option value="">${itemOptTitle}</option>
			<c:forEach var="optnDt" items="${optnOptList}" varStatus="i">
				<option value="<c:out value="${optnDt.OPTION_CODE}"/>" data-pcode="${optnDt.PARENT_OPTION_CODE}" data-level="${optnDt.OPTION_LEVEL}" <c:if test="${!empty optnDt.P_OPT_CD}"> data-p="<c:out value="${optnDt.P_OPT_CD}"/>"</c:if><c:if test="${optnDt.OPTION_CODE == objVal || empty objVal && optnDt.OPTION_CODE == defVal}"> selected="selected"</c:if><c:if test="${optnDt.OPTION_CODE == defVal}"> data-default="1"</c:if>><c:if test="${addOrder}">${i.count}. </c:if><c:out value="${optnDt.OPTION_NAME}"/><c:if test="${itemId eq 'ncsSclasCode'}"><c:if test="${optnDt.OPTION_LEVEL eq optMaxLevel}">(<c:out value="${optnDt.OPTION_CODE}"/>)</c:if></c:if></option>
			</c:forEach>
			</select>
		</c:otherwise>
	</c:choose>
