...
<c:if test="${empty isSearchList}">
	...
	<c:if test="${!empty itemListSearch}">
		...
		<c:set var="itemList" value="${fn:split(itemSearchOrder, ',')}"/>
		<c:set var="itemListLength" value="${fn:length(itemList)}" />
									...
									<c:if test="${itemSearchId == keyFieldItemName}">
										...
										<dl>
											...
											<dd>
												...
												<input type="text" name="${keyFieldInfo.query_name}" id="${keyFieldInfo.query_name}" value="<c:out value="${queryString[keyFieldInfo.query_name]}"/>" title="검색어" class="inputSearchText"/>
												</div>
											</dd>
										</dl>
										</c:if>
