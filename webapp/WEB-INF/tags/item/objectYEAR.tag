...
<c:if test="${!empty itemObj}">
	...
	<select id="${id}" name="${name}" title="${itemName}" style="${objStyle}"<c:if test="${required == 1}"> required="required"</c:if> class="${objClass}">
...
<c:if test="${empty itemObj.module_id || itemObj.module_id ne 'traing'}">${itemYearName}</c:if>
</c:if>