...
<dt><itui:objectLabelForArea itemId="${itemId}" required="${required}" itemInfo="${itemInfo}"/></dt>
<dd<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
<itui:objectYEARForArea defVal="${defVal}" itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objStyle="${objStyle}" objVal="${objVal}"/>
</dd>
</c:if>