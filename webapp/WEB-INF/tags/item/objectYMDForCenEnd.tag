...
<c:if test="${!empty itemObj}">
	...
	<div class="input-calendar-area">		
		<input type="text" id="endDate" name="" title="${itemName}" class="edate" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${objVal}"/>"/>
	</div>
</c:if>