...
<c:if test="${objVal == null}"><c:set var="objVal" value="${objDt[itemColumnId]}"/></c:if>
	...
	<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if> class="left">
	<itui:objectViewCustom itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" defVal="${defVal}" objVal="${objVal}" optnHashMap="${optnHashMap}" downloadUrl="${downloadUrl}" classSplitStr="${classSplitStr}"/>
	</td>
</c:if>