...
<c:if test="${!empty itemObj}">
	...
	<ul class="width100 ${ulClass}">
		<li class="fileBox">
			<input type="text" class="fileName" placeholder="[수요조사지, 환경분석 자료 등 컨설팅 수행 시 활용자료 첨부]" title="수요조사지, 환경분석 자료 등 컨설팅 수행 시 활용자료 첨부" readonly>
			<span id="findFile_${itemId}"><label for="${itemId}" class="btn_file">찾아보기</label></span>
		...
		<c:if test="${!empty fileList}">
			<c:set var="keyItemId" value="${settingInfo.diary_name}"/>
			<c:set var="keyColumnId" value="${settingInfo.diary_column}"/>
		<li class="fn_multiFileDown" style="text-align:left; padding:10px 0 0 0;">
			<dl class="saved_file_nb">
				<dt><c:if test="${crtSiteId  == 'web' }"><spring:message code="item.file.saved.name"/></c:if></dt>
				<dd>
				<ul>
				<c:forEach var="optnDt" items="${fileList}" varStatus="i">
					<li><a href="diaryDownload.do?mId=99&${keyItemId}=<c:out value="${optnDt[keyColumnId]}"/>&fidx=<c:out value="${optnDt.FLE_IDX}"/>&itId=<c:out value="${itemId}"/>"><img src="../images/common/ico_file.gif" alt="파일"><c:out value="${optnDt.FILE_ORIGIN_NAME}"/></a></li>
				</c:forEach>
				</ul>
				</dd>
			</dl>
