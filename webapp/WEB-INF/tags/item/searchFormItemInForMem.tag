					...
					<c:forEach var="itemSearchId" items="${itemSearchOrder}">
						...
						<c:choose>
							...
							<div class="input-group">		
								<input type="text" id="${itemKeyId}1" name="${itemKeyId}1" title="${titleStart}" class="${objClass}" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${queryString[value1Name]}"/>"/>
							...
							<c:out value="~"/>
							<div class="input-group">		
								<input type="text" id="${itemKeyId}2" name="${itemKeyId}2" title="${titleEnd}" class="${objClass}" style="${objStyle}" readonly="readonly"<c:if test="${required == 1}"> required="required"</c:if> value="<c:out value="${queryString[value2Name]}"/>"/>
								<span><button type="button" id="fn_btn_${itemKeyId}2" title="${titleEnd}" class="${btnClass}" data-idName="${itemKeyId}2"><c:out value="종료일"/></button></span>
							</div>
							</c:when>
							...
							<c:choose>
								<c:when test="${!empty keyItem.option_type && keyItem.option_type != 0}"><c:set var="optnOptList" value="${searchOptnHashMap[itemSearchId]}"/></c:when>
								<c:when test="${!empty authGroup}"><c:set var="optnOptList" value="${authGroup}"/></c:when>
