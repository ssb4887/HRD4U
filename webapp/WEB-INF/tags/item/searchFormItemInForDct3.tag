			...
			<c:if test="${!empty itemListSearch}">
				...
				<c:if test="${!empty itemSearchOrder}">
					...
					<c:forEach var="itemSearchId" items="${itemSearchOrder}">
						...
						<div class="half-box">
						<dl>
						<dt id="fn_dt_${itemKeyId}" class="${moreClassName}" style="padding: 0 0 0 15px;"><label for="${itemKeyId}">${itemLabelName}</label></dt>
						...
						<c:choose>
							...
							<select name="${itemKeyId}" id="${itemKeyId}" class="select" style=" width:50%;" title="${itemKeyName}"<c:if test="${!empty keyItem.style_width}"> style="width:${keyItem.style_width}%;"</c:if>>
