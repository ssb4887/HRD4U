...
<td<c:if test="${colspan > 0}"> colspan="${colspan}"</c:if>>
	<itui:objectYMD itemId="${itemId}" itemInfo="${itemInfo}" objDt="${objDt}" objVal="${objVal}" btnId="${btnId}" btnClass="${btnClass}" btnText="${btnText}" objClass="${objClass}" objStyle="${objStyle}"/>
</td>
</c:if>