...
<%@ attribute name="noItemInfo"%>
<% /* 이메일 */ %>
<c:set var="itemObj" value="${itemInfo.items[itemId]}"/>
<c:if test="${!empty itemObj || !empty noItemInfo}">
	...
	<c:if test="${empty objStyle1}"><c:set var="objStyle1" value="width:30%;"/></c:if>
	<c:if test="${empty objStyle2}"><c:set var="objStyle2" value="width:30%;"/></c:if>
	<c:if test="${empty objStyle3}"><c:set var="objStyle3" value="width:28%;"/></c:if>
	...
	<div class="input-email">
		...
		<c:set var="emailOpts" value="daum.net,gmail.com,hanmail.net,icloud.com,kakao.com,lycos.co.kr,msn.com,nate.com,naver.com,yahoo.com"/>
		...
		<select id="${itemId}3" name="${itemId}3" title="<spring:message code="item.itemId.email.second.select.name" arguments="${itemName}"/>" class="${objClass3}" style="${objStyle3}"<c:if test="${required == 1}"> required="required"</c:if>>
			...
			<option value="0"<c:if test="${!empty objVal2 && !isOptionSel}"> selected="selected"</c:if>>직접입력</option>
...
<script type="text/javascript">
	...
	$("#" + itemId + "3").change(function () {
		var domain = $(this).val();
		if(domain != "0"){
			$("#" + itemId + "2").attr("readonly", true);
			$("#" + itemId + "2").val(domain);
		} else {
			$("#" + itemId + "2").attr("readonly", false);
		}
	});
</script>