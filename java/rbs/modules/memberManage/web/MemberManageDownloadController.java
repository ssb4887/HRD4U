 ...
 */
@Controller
@ModuleMapping(moduleId="memberManage")
@RequestMapping({"/{siteId}/memberManage", "/{admSiteId}/menuContents/{usrSiteId}/memberManage"})
public class MemberManageDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = memberManageService.getMultiFileView(param);
			if(dt != null) {
				int result = memberManageService.updateMultiFileDown(keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = memberManageService.getFileView(param);
			if(dt != null) {
				int result = memberManageService.updateFileDown(keyIdx, columnId);
			}
		}
