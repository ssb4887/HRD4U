...
public class AlarmDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = alarmService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				int result = alarmService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = alarmService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = alarmService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
