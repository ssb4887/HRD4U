package rbs.modules.alarm.service.impl;
...
import rbs.modules.alarm.mapper.AlarmFileMapper;
import rbs.modules.alarm.mapper.AlarmMapper;
import rbs.modules.alarm.mapper.AlarmMultiMapper;
import rbs.modules.alarm.service.AlarmService;
...
public class AlarmServiceImpl extends EgovAbstractServiceImpl implements AlarmService {
	...
	@Resource(name="alarmFileMapper")
	private AlarmFileMapper alarmFileDAO;
	
	@Resource(name="alarmMultiMapper")
	private AlarmMultiMapper alarmMultiDAO;
	...
	public int getTotalCount(int fnIdx, Map<String, Object> param) {
    	...
    	return alarmDAO.getTotalCount(param);
    }
	...
	public DataMap getView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = alarmDAO.getView(param);
		return viewDAO;
	}
	...
	public DataMap getFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = alarmDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getMultiFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = alarmFileDAO.getFileView(param);
		return viewDAO;
	}
	...
	public int updateFileDown(int fnIdx, int keyIdx, String fileColumnName) throws Exception{
		...
		return alarmDAO.updateFileDown(param);
	...
	public int updateMultiFileDown(int fnIdx, int keyIdx, int fleIdx, String itemId) throws Exception{
    	...
    	param.put("fnIdx", fnIdx);
		param.put("KEY_IDX", keyIdx);
		return alarmFileDAO.updateFileDown(param);
	...
	public DataMap getModify(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = alarmDAO.getModify(param);
		return viewDAO;
	}
	...
	public int getAuthCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return alarmDAO.getAuthCount(param);
    }
	...
	public int insert(String uploadModulePath, int fnIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception  {
		...
		// 1. key 얻기
		param.put("fnIdx", fnIdx);
		int keyIdx = alarmDAO.getNextId(param);
		...
		//dataList.add(new DTForm("MEMBER_IDX", loginMemberIdx));
		...
		// 3. DB 저장
		// 3.1 기본 정보 테이블
		int result = alarmDAO.insert(param);
		...
		if(result > 0) {
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = alarmMultiDAO.insert(fileParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("fnIdx", fnIdx);
					fileParam.put("KEY_IDX", keyIdx);
					int fleIdx = alarmFileDAO.getNextId(fileParam);
					fileParam.put("FLE_IDX", fleIdx);
					result = alarmFileDAO.insert(fileParam);
				}
			}
		}
	...
	public int update(String uploadModulePath, int fnIdx, int keyIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
    	...
    	//이력남기기
    	chgLogMapper.writeDataLog("HRD_SPTJ_BOARD_NOTIFICATION","U", searchList);
    	
    	// 3. DB 저장
    	// 3.1 기본 정보 테이블
		int result = alarmDAO.update(param);
		...
		if(result > 0){
			...
			result1 = alarmMultiDAO.cdelete(multiDelParam);
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = alarmMultiDAO.insert(multiParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				int fleIdx = alarmFileDAO.getNextId(fileParam1);*/
			...
			if(fileModifyDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					fileParam.put("fnIdx", fnIdx);
					result = alarmFileDAO.update(fileParam);
				}
			}
			...
			if(fileDeleteSearchList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					// 6.1 삭제목록 select
					List<Object> deleteFileList2 = alarmFileDAO.getList(fileParam);
					if(deleteFileList2 != null) deleteMultiFileList.addAll(deleteFileList2);
					// 6.2 DB 삭제
					result = alarmFileDAO.cdelete(fileParam);
				}
	...
	public int getDeleteCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return alarmDAO.getDeleteCount(param);
    }
	...
	public List<Object> getDeleteList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return alarmDAO.getDeleteList(param);
	}
	...
	public int delete(int fnIdx, ParamForm parameterMap, String regiIp, JSONObject settingInfo) throws Exception {
		...
		searchList.add(new DTForm("MEMBER_IDX", loginMemberIdx));
    	...
    	//이력남기기
    	chgLogMapper.writeDataLog("HRD_SPTJ_BOARD_NOTIFICATION","D", searchList);
		
		// 3. DB 저장
		return alarmDAO.delete(param);
	}
	...
	public int cdelete(String uploadModulePath, int fnIdx, int[] deleteIdxs, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		deleteMultiFileList = alarmFileDAO.getList(param);
		...
		if(deleteFileColumnList != null) {
			...
			deleteFileList = alarmDAO.getFileList(param);
		}
		
		// 3. delete
		int result = alarmDAO.cdelete(param);
		if(result > 0) {
			// 4. 파일 삭제
			// 4.1 multi file 삭제
			String fileRealPath = RbsProperties.getProperty("Globals.upload.file.path") + File.separator + uploadModulePath;
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && objectType == 9) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, alarmFileDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
				...
				param.put("searchList", searchList);
		    	param.put("fnIdx", fnIdx);
				resultHashMap = alarmFileDAO.getMapList(param);
			}
		}
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && (objectType == 3 || objectType == 4 || objectType == 11)) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, alarmMultiDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
				...
				param.put("searchList", searchList);
		    	param.put("fnIdx", fnIdx);
				resultHashMap = alarmMultiDAO.getMapList(param);
			}
		}
