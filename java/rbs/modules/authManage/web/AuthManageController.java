package rbs.modules.authManage.web;
 ...
 */
@Controller
@ModuleMapping(moduleId = "authManage", useSDesign = true)
@RequestMapping({ "/{siteId}/authManage", "/{admSiteId}/menuContents/{usrSiteId}/authManage" })
public class AuthManageController extends ModuleController {
	 ...
	 */
	public HashMap<String, Object> getOptionHashMap(String submitType, JSONObject itemInfo,
			HashMap<String, Object> addOptionHashMap) {
		// 코드
		...
		HashMap<String, Object> optnHashMap = (addOptionHashMap != null)
				? CodeHelper.getItemOptnHashMap(items, itemOrder, addOptionHashMap)
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. 페이지정보 setting
		int listUnit = JSONObjectUtil.getInt(settingInfo, "dset_list_count",
		...
		if (pageUnit == 0)
			pageUnit = listUnit; // 페이지당 목록 수
		int pageSize = JSONObjectUtil.getInt(settingInfo, "dset_list_block", propertiesService.getInt("PAGE_SIZE")); // 페이징
																														// 크기
		...
		// 2.2 목록수
		totalCount = authManageService.getTotalCount(param);
		...
		if (totalCount > 0) {
			...
			// 2.3 목록
			list = authManageService.getList(param);
			authList = authManageService.getCodeList(param);
		}
		...
		model.addAttribute("authList", authList); // 목록
		...
		if (searchAddOptnHashMap != null) {
			if (searchOptnHashMap == null)
				searchOptnHashMap = new HashMap<String, Object>();
			searchOptnHashMap.putAll(searchAddOptnHashMap);
		}
		model.addAttribute("searchOptnHashMap", searchOptnHashMap); // 검색항목코드

		// 권한그룹 가져오기
		List<Object> authGroup = null;
		authGroup = authManageService.getAuthGroupList();
	...
	@Auth(role = Role.R)
	@ModuleAuth(name = "LST")
	@RequestMapping(value = "/authGrant.do")
	public String authGrant(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model)
			throws Exception {
		DataForm queryString = attrVO.getQueryString();
		...
		String cateTabColumnId = JSONObjectUtil.getString(JSONObjectUtil.getJSONObject(items, cateTabItemId),
				"column_id");
		String cateTabMasterCode = JSONObjectUtil.getString(JSONObjectUtil.getJSONObject(items, cateTabItemId),
				"master_code");
		int cateTabObjectType = JSONObjectUtil.getInt(settingInfo, "dset_cate_tab_object_type");
		// 검색 코드
		HashMap<String, Object> searchOptnHashMap = CodeHelper.getItemOptnSearchHashMap(listSearch, cateTabItemId,
				cateTabColumnId, cateTabObjectType, cateTabMasterCode);
		// 목록 코드
		HashMap<String, Object> optnHashMap = getOptionHashMap(submitType, itemInfo, searchOptnHashMap);// CodeHelper.getItemOptnHashMap(items,
																										// itemOrder,
																										// searchOptnHashMap);
		// 탭 검색 조건
		List<DTForm> cateTabSearchList = ModuleUtil.getCateTabSearchList(settingInfo, items, queryString, optnHashMap);// fn_getCateTabSearchList(queryString,
																														// settingInfo,
																														// items,
																														// optnHashMap);
		...
		if (totalCount > 0) {
			...
			// 2.3 목록
			list = authManageService.getAuthGrant(param);
		}
		...
		return getViewPath("/authGrant");
	}
	
	@Auth(role = Role.R)
	@ModuleAuth(name = "LST")
	@RequestMapping("/memberAuth.do")
	public String memberAuth(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response,
		...
		/*
		 * // 전체관리권한여부 Boolean isMngAuth = AuthHelper.isAuthenticated("MNG"); // 전체관리
		 * if(!isMngAuth){ LoginVO loginVO = (LoginVO)
		 * UserDetailsHelper.getAuthenticatedUser(); String loginMemberIdx = null;
		...
		// 상세정보
		dt = authManageService.getView(param);
		if (dt == null) {
			// 해당글이 없는 경우
		...
		model.addAttribute("multiFileHashMap",
				authManageService.getMultiFileHashMap(keyIdx, settingInfo, items, itemOrder)); // multi file 목록
		model.addAttribute("multiDataHashMap",
				authManageService.getMultiHashMap(keyIdx, settingInfo, items, itemOrder)); // multi file 목록
	...
	public String input(@RequestParam(value = "mode") String mode, @ModuleAttr ModuleAttrVO attrVO,
		...
		// 2.2 상세정보
		dtMem = authManageService.getModify(param);
		searchList = new ArrayList<DTForm>();
		searchList.add(new DTForm("A.MEMBER_IDX", dtMem.get("MEMBER_IDX")));
		searchList.add(new DTForm("A.AUTH_MEMBER_IDX", dtMem.get("AUTH_MEMBER_IDX")));
		param.put("searchList", searchList);
		dt = authManageService.getMember(param);
		list = authManageService.getAuthGroupList();
		if (list == null) {
			// 해당글이 없는 경우
			model.addAttribute("message", MessageUtil.getAlert(rbsMessageSource.getMessage("message.no.contents")));
			return RbsProperties.getProperty("Globals.fail.path");
		...
		model.addAttribute("multiDataHashMap",
				authManageService.getMultiHashMap(keyIdx, settingInfo, items, itemOrder)); // multi file 목록
		// model.addAttribute("multiFileHashMap",
		// authManageService.getMultiFileHashMap(keyIdx, settingInfo, items,
		// itemOrder)); // multi file 목록
		model.addAttribute("optnHashMap", CodeHelper.getItemOptnHashMap(items, itemOrder)); // 항목코드
		model.addAttribute("submitType", submitType); // 페이지구분
	...
	public String inputGroup(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model)
			throws Exception {
		JSONObject itemInfo = attrVO.getItemInfo();
	...
	public String inputProc(@RequestParam(value = "mode") String mode, @ParamMap ParamForm parameterMap,
		...
		dt = authManageService.getModify(param);
		if (dt == null) {
			// 해당글이 없는 경우
		...
		// 3.2 필수입력 체크
		String errorMessage = FormValidatorUtil.getErrorMessage(ajaxPName, parameterMap, moduleValidator,
				new Object[] { isAdmMode, inputFlag, items, itemOrder, settingInfo });
		...
		// 4. DB
		int result = authManageService.update(uploadModulePath, keyIdx, request.getRemoteAddr(), parameterMap,
		...
		if (result > 0) {
			...
			model.addAttribute("message", MessageUtil.getAlertAddWindow(isAjax,
					rbsMessageSource.getMessage("message.insert"), "self.close();"));
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO,
		...
		authManageService.apply(parameterMap, keyIdx, request.getRemoteAddr(), settingInfo);
		// 2. DB
		int result = authManageService.insert(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo,
				items, itemOrder);
		if (result > 0) {
			// 2.1 저장 성공
			// 기본경로
	...
	public String inputGroupProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO,
			HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		boolean isAdmMode = attrVO.isAdmMode();
		JSONObject settingInfo = attrVO.getSettingInfo();
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value = "select") String[] deleteIdxs,
			@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		JSONObject settingInfo = attrVO.getSettingInfo();
		String ajaxPName = attrVO.getAjaxPName();
		...
		// 1. DB
		int result = authManageService.delete(parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);

		if (result > 0) {
			// 1.1 저장 성공
			model.addAttribute("message", MessageUtil.getAlert(isAjax, rbsMessageSource.getMessage("message.delete"),
					"top.location.reload();"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		}
		// 1.2 저장 실패
	...
	public String delete(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request,
			ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		String ajaxPName = attrVO.getAjaxPName();
		JSONObject settingInfo = attrVO.getSettingInfo();
		...
		if (result > 0) {
			...
			fn_setCommonPath(attrVO);
			model.addAttribute("message",
					MessageUtil.getAlertAddWindow(isAjax, rbsMessageSource.getMessage("message.delete"),
							"fn_procAction('"
									+ PathUtil.getAddProtocolToPagePath(
											StringUtil.getString(request.getAttribute("URL_LIST")))
									+ "', " + StringUtil.getInt(request.getParameter("mdl")) + ");"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		}
		// 3.2 저장 실패
		model.addAttribute("message", MessageUtil.getAlert(isAjax, rbsMessageSource.getMessage("message.no.delete")));
	...
	public String restore(@RequestParam(value = "select") String[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO,
			HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = authManageService.restore(restoreIdxs, request.getRemoteAddr(), settingInfo);

		if (result > 0) {
			// 1.1 저장 성공
			model.addAttribute("message", MessageUtil.getAlertAddWindow(isAjax,
					rbsMessageSource.getMessage("message.restore"), "fn_procReload();"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		}
		// 1.2 저장 실패
	...
	public String cdelete(@RequestParam(value = "select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO,
			HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = authManageService.cdelete(uploadModulePath, deleteIdxs, settingInfo, items, itemOrder);

		if (result > 0) {
			// 1.2.1 저장 성공
			model.addAttribute("message", MessageUtil.getAlertAddWindow(isAjax,
					rbsMessageSource.getMessage("message.cdelete"), "location.reload();"));
			return RbsProperties.getProperty("Globals.message" + ajaxPName + ".path");
		}
		// 1.2.2 저장 실패
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model)
			throws Exception {
		JSONObject settingInfo = attrVO.getSettingInfo();
		JSONObject itemInfo = attrVO.getItemInfo();
		DataForm queryString = attrVO.getQueryString();
		...
		List<DTForm> itemInfoSearchList = ModuleUtil.getItemInfoDeleteSearchList("A.LAST_MODI_DATE",
				deleteListSearchParams, listSearch, queryString);
		if (itemInfoSearchList != null) {
			searchList.addAll(itemInfoSearchList);
			model.addAttribute("isSearchList", new Boolean(true));
		}
		...
		// 2.2 목록수
		totalCount = authManageService.getDeleteCount(param);
		paginationInfo.setTotalRecordCount(totalCount);
		...
		if (totalCount > 0) {
			...
			// 2.3 목록
			list = authManageService.getDeleteList(param);
		}
	...
	public ModelAndView fileUpload(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO,
			HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		if (parameterMap.get(submitType) == null) {
			model.addAttribute("message",
					MessageUtil.getAlert(isAjax, rbsMessageSource.getMessage("message.error.data")));
			return mav;
		}

		// 2. DB
		Map<String, Object> fileInfo = authManageService.getFileUpload(uploadModulePath, request.getRemoteAddr(),
				parameterMap, settingInfo, items, itemOrder);
		if (fileInfo != null && !fileInfo.isEmpty()) {
			// 2.1 저장 성공
			model.addAttribute("message",
					MessageUtil.getAlertAddWindow(isAjax, rbsMessageSource.getMessage("message.insert"), ""));
	 ...
	 */
	@Auth(role = Role.U)
	@RequestMapping(method = RequestMethod.GET, value = "/updateAll.do")
	public ModelAndView updateAll(@ModuleAttr ModuleAttrVO attrVO, @RequestBody String json, HttpServletRequest request, HttpServletResponse response) throws Exception {
		String authIdx = request.getParameter("authIdx");
		String memberList = request.getParameter("memberList");
		int result = authManageService.updateAll(authIdx, memberList);
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {
		...
		String[] searchParams = PathUtil.getSearchParams(JSONObjectUtil.getJSONObject(itemInfo, listSearchId)); // 목록 검색
																												// 항목
		...
		if (!StringUtil.isEmpty(cateTabId))
			tabBaseParams = new String[] { RbsProperties.getProperty("Globals.item.tab.search.pre.flag") + cateTabId };
		...
		PathUtil.fn_setCommonPath(queryString, baseParams, tabBaseParams, searchParams, null, null, pageName, idxName,
				listName, viewName, null, null, null, null, inputName, inputProcName, deleteProcName, deleteListName,
				imageName, movieName, downloadName);
		
		// 추가경로
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.currentRequestAttributes()).getRequest();
		String baseQueryString = StringUtil.getString(request.getAttribute("baseQueryString"));
		
		String updateAllUrl = "updateAll.do";				// 일괄변경
		if(!StringUtil.isEmpty(baseQueryString)) {
			updateAllUrl += baseQueryString;
		}
		request.setAttribute("URL_UPDATEALL", updateAllUrl);
	...
	public void fn_setAddCommonPath(ModuleAttrVO attrVO, HttpServletRequest request) {
		...
		String inputGroupName = "inputGroup.do";
		String inputGroupProcName = "inputGroupProc.do";
		...
		String inputGroupNameUrl = inputGroupName;
		String inputGroupProcNameUrl = inputGroupProcName;
		String excelNameUrl = excelName;

		if (!StringUtil.isEmpty(baseQueryString)) {
			memberSearchListNameUrl += baseQueryString;
			inputGroupNameUrl += baseQueryString;
			inputGroupProcNameUrl += baseQueryString;
			excelNameUrl += baseQueryString;
		}

		request.setAttribute("URL_INPUTGROUP", inputGroupNameUrl);
		request.setAttribute("URL_INPUTGROUPPROC", inputGroupProcNameUrl);
	...
	public void fn_setDeleteListPath(ModuleAttrVO attrVO) {
		...
		// 항목 설정 정보
		String listSearchId = "list_search";
		String[] searchParams = StringUtil.concatenateStringArrays(
				PathUtil.getSearchParams(JSONObjectUtil.getJSONObject(itemInfo, listSearchId)), deleteListSearchParams); // 검색
																															// 항목
	...
	public boolean isMngProcAuth(JSONObject settingInfo, String keyIdx) {
		...
		if (!isMngAuth) {
			...
			param.put("searchList", searchList);
			modiCnt = authManageService.getAuthCount(param);
