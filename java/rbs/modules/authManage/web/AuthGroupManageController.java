 ...
 */
@Controller
@ModuleMapping(moduleId="authGroupManage", confModule="authManage", confSModule="authGroupManage")
@RequestMapping({"/{siteId}/authGroupManage", "/{admSiteId}/menuContents/{usrSiteId}/authGroupManage"})
public class AuthGroupManageController extends ModuleController {
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = authGroupManageService.getTotalCount(param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = authGroupManageService.getList(param);
		}
    	
    	groupList = authGroupManageService.getGroupList(param);

    	// 3. 속성 setting
    	model.addAttribute("paginationInfo", paginationInfo);										// 페이징정보
    	model.addAttribute("list", list);															// 목록
    	model.addAttribute("authGroup", groupList);															// 목록
	...
	public String authGrant(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		DataForm queryString = attrVO.getQueryString();
		boolean isAdmMode = attrVO.isAdmMode();
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = authGroupManageService.getAuthGrant(param);
		}
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 상세정보
		dt = authGroupManageService.getView(param);
		if(dt == null) {
			// 해당글이 없는 경우
		...
		model.addAttribute("multiFileHashMap", authGroupManageService.getMultiFileHashMap(keyIdx, settingInfo, items, itemOrder));	// multi file 목록
		model.addAttribute("multiDataHashMap", authGroupManageService.getMultiHashMap(keyIdx, settingInfo, items, itemOrder));		// multi file 목록
	...
	public String input(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 상세정보
		dt = authGroupManageService.getModify(param);
		//dtList = authGroupManageService.getModify(param);
		list = authGroupManageService.getAuthList();
		if(list == null) {
			// 해당글이 없는 경우
			model.addAttribute("message", MessageUtil.getAlert(rbsMessageSource.getMessage("message.no.contents")));
			return RbsProperties.getProperty("Globals.fail.path");
	...
	public String inputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 4. DB
    	int result = authGroupManageService.update(uploadModulePath, keyIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 4.1 저장 성공
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
    	int result = authGroupManageService.insert(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	@Auth(role = Role.R)
	@ModuleAuth(name="WRT")
	@RequestMapping(method=RequestMethod.GET, value = "/getAuthList.do")
	public String getAuthList(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		String result ="";
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = authGroupManageService.delete(parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String restore(@RequestParam(value="select") String[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = authGroupManageService.restore(restoreIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String cdelete(@RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = authGroupManageService.cdelete(uploadModulePath, deleteIdxs, settingInfo, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = authGroupManageService.getDeleteCount(param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = authGroupManageService.getDeleteList(param);
    	}
	...
	public ModelAndView fileUpload(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		Map<String, Object> fileInfo = authGroupManageService.getFileUpload(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
	...
	public boolean isMngProcAuth(JSONObject settingInfo, String keyIdx) {
		...
		if(!isMngAuth){
			...
			param.put("searchList", searchList);
			modiCnt = authGroupManageService.getAuthCount(param);
