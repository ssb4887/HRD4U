 ...
 */
@Controller
@ModuleMapping(moduleId="authManage")
@RequestMapping({"/{siteId}/authManage", "/{admSiteId}/menuContents/{usrSiteId}/authManage"})
public class AuthManageDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = authManageService.getMultiFileView(param);
			if(dt != null) {
				//int result = authManageService.updateMultiFileDown(keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = authManageService.getFileView(param);
			if(dt != null) {
				//int result = authManageService.updateFileDown(keyIdx, columnId);
			}
		}
