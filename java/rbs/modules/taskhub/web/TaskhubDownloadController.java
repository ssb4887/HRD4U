...
public class TaskhubDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = taskhubService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				taskhubService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = taskhubService.getFileView(fnIdx, param);
			if(dt != null) {
				taskhubService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
