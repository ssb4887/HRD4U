...
public class SampleDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = sampleService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				int result = sampleService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = sampleService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = sampleService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
