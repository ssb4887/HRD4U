...
public class AreaActDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			//dt = areaActService.getMultiFileView(fnIdx, param);
			/*if(dt != null) {
				//int result = areaActService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {*/
			...
			param.put("searchList", searchList);
			dt = areaActService.getFileView(fnIdx, param);
			/*if(dt != null) {
				//int result = areaActService.updateFileDown(fnIdx, keyIdx, columnId);
			}*/
		}
