...
public class BoardServiceImpl extends EgovAbstractServiceImpl implements BoardService {
	 ...
	 * @throws Exception
	 */
	public String getIdx(int fnIdx, int brdIdx) throws Exception {
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("fnIdx", fnIdx);
    	param.put("BRD_IDX", brdIdx);
		return boardDAO.getIdx(param);
	}
    	...
    	*/
		...
		if(fnIdx == 1) {
			try {
				...
				String location = StringUtil.getString(parameterMap.get("location"));
				if(!StringUtil.isEquals(location, "17")) {
					mailParam.put("SIDO", location);
				}
							
				// 연락처 목록 가져오기
	    		resultMap = alarmDAO.getMailList(mailParam);
	    		List resultList = StringUtil.getList(resultMap);
	    		int resultSize = resultList.size();
	    		
	    		String title = "공지사항 알림톡";
	    		String SIDO = "";
	    		String templateCode = "SJB_076909";
	    		...
	    		for(int i = 0 ; i < resultSize ; i ++) {
	    			...
	    			if(StringUtil.isEquals(location, "17")) {
	    				...
	    				SIDO = StringUtil.getString(resultParam.get("SIDO"));
	    			}
	    			
	    			
		    		String msg = ""; 
		    		msg += "[한국산업인력공단] ";
		    		msg += SIDO +" 공지사항 게시글이 등록되었습니다.";
	    		
	    			map.put("userCode", "board" + idx + i);               // 알림톡 PK
	...
	public int update(String uploadModulePath, String boardDesignType, int fnIdx, int brdIdx, String siteMode, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		if(!StringUtil.isEquals(boardDesignType, RbsProperties.getProperty("Globals.design.NAME_DESIGN_TYPE_MEMO"))){
			...
			boardMultiDAO.cdelete(multiDelParam);
		}
