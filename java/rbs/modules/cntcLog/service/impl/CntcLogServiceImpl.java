package rbs.modules.cntcLog.service.impl;
...
import rbs.modules.cntcLog.mapper.CntcLogFileMapper;
import rbs.modules.cntcLog.mapper.CntcLogMapper;
import rbs.modules.cntcLog.mapper.CntcLogMultiMapper;
 ...
 *
 */
@Service("cntcLogService")
public class CntcLogServiceImpl extends EgovAbstractServiceImpl implements CntcLogService {

	@Resource(name="cntcLogMapper")
	private CntcLogMapper cntcLogDAO;
	
	@Resource(name="cntcLogFileMapper")
	private CntcLogFileMapper cntcLogFileDAO;
	
	@Resource(name="cntcLogMultiMapper")
	private CntcLogMultiMapper cntcLogMultiDAO;
	 ...
	 */
    @Override
	public int getTotalLogCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return cntcLogDAO.getTotalLogCount(param);
    }
	 ...
	 */
	@Override
	public List<Object> getLogList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return cntcLogDAO.getLogList(param);
	}
	...
	public int getTotalCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return cntcLogDAO.getTotalCount(param);
    }
	...
	public List<Object> getList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return cntcLogDAO.getList(param);
	}
	...
	public DataMap getView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = cntcLogDAO.getView(param);
		return viewDAO;
	}
	...
	public DataMap getFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = cntcLogDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getMultiFileView(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = cntcLogFileDAO.getFileView(param);
		return viewDAO;
	}
	...
	public int updateFileDown(int fnIdx, int keyIdx, String fileColumnName) throws Exception{
		...
		return cntcLogDAO.updateFileDown(param);
	...
	public int updateMultiFileDown(int fnIdx, int keyIdx, int fleIdx, String itemId) throws Exception{
    	...
    	param.put("fnIdx", fnIdx);
		param.put("KEY_IDX", keyIdx);
		return cntcLogFileDAO.updateFileDown(param);
	...
	public DataMap getModify(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		DataMap viewDAO = cntcLogDAO.getModify(param);
		return viewDAO;
	}
	...
	public int getAuthCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return cntcLogDAO.getAuthCount(param);
    }
	...
	public int insert(String uploadModulePath, int fnIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception  {
		...
		// 1. key 얻기
		param.put("fnIdx", fnIdx);
		int keyIdx = cntcLogDAO.getNextId(param);
		...
		// 3. DB 저장
		// 3.1 기본 정보 테이블
		int result = cntcLogDAO.insert(param);
		...
		if(result > 0) {
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = cntcLogMultiDAO.insert(fileParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("fnIdx", fnIdx);
					fileParam.put("KEY_IDX", keyIdx);
					int fleIdx = cntcLogFileDAO.getNextId(fileParam);
					fileParam.put("FLE_IDX", fleIdx);
					result = cntcLogFileDAO.insert(fileParam);
				}
			}
		}
	...
	public int update(String uploadModulePath, int fnIdx, int keyIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
    	...
    	// 3. DB 저장
    	// 3.1 기본 정보 테이블
		int result = cntcLogDAO.update(param);
		...
		/*if(result > 0){
			...
			result1 = cntcLogMultiDAO.cdelete(multiDelParam);
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = cntcLogMultiDAO.insert(multiParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				int fleIdx = cntcLogFileDAO.getNextId(fileParam1);
			...
			if(fileModifyDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					fileParam.put("fnIdx", fnIdx);
					result = cntcLogFileDAO.update(fileParam);
				}
			}
			...
			if(fileDeleteSearchList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					// 6.1 삭제목록 select
					List<Object> deleteFileList2 = cntcLogFileDAO.getList(fileParam);
					if(deleteFileList2 != null) deleteMultiFileList.addAll(deleteFileList2);
					// 6.2 DB 삭제
					result = cntcLogFileDAO.cdelete(fileParam);
				}
	...
	public int getDeleteCount(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
    	return cntcLogDAO.getDeleteCount(param);
    }
	...
	public List<Object> getDeleteList(int fnIdx, Map<String, Object> param) {
    	param.put("fnIdx", fnIdx);
		return cntcLogDAO.getDeleteList(param);
	}
	...
	public int delete(int fnIdx, ParamForm parameterMap, int[] deleteIdxs, String regiIp, JSONObject settingInfo) throws Exception {
		...
		// 3. DB 저장
		return cntcLogDAO.delete(param);
	}
	...
	public int restore(int fnIdx, int[] restoreIdxs, String regiIp, JSONObject settingInfo) throws Exception {
		...
		// 3. DB 저장
		return cntcLogDAO.restore(param);
	}
	...
	public int cdelete(String uploadModulePath, int fnIdx, int[] deleteIdxs, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		deleteMultiFileList = cntcLogFileDAO.getList(param);
		...
		if(deleteFileColumnList != null) {
			...
			deleteFileList = cntcLogDAO.getFileList(param);
		}
		
		// 3. delete
		int result = cntcLogDAO.cdelete(param);
		if(result > 0) {
			// 4. 파일 삭제
			// 4.1 multi file 삭제
			String fileRealPath = RbsProperties.getProperty("Globals.upload.file.path") + File.separator + uploadModulePath;
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && objectType == 9) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, cntcLogFileDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
				...
				param.put("searchList", searchList);
		    	param.put("fnIdx", fnIdx);
				resultHashMap = cntcLogFileDAO.getMapList(param);
			}
		}
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && (objectType == 3 || objectType == 4 || objectType == 11)) {
					...
					param.put("searchList", searchList);
			    	param.put("fnIdx", fnIdx);
					resultHashMap.put(itemId, cntcLogMultiDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
				...
				param.put("searchList", searchList);
		    	param.put("fnIdx", fnIdx);
				resultHashMap = cntcLogMultiDAO.getMapList(param);
			}
		}
