package rbs.modules.cntcLog.web;
...
import rbs.modules.cntcLog.service.CntcLogService;
 ...
 */
@Controller
@ModuleMapping(moduleId="cntcLog")
@RequestMapping({"/{siteId}/cntcLog", "/{admSiteId}/menuContents/{usrSiteId}/cntcLog"})
public class CntcLogController extends ModuleController {
	
	@Resource(name = "cntcLogService")
	private CntcLogService cntcLogService;
	...
	public String logList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(date1Values != null) {
			...
			if(!date1.isEmpty()) {
				param.put("OPERT_START_DT", date1);
				model.addAttribute("OPERT_START_DT", date1);
				model.addAttribute("isSearchList", new Boolean(true));
			}
		}
		...
		if(date2Values != null) {
			...
			if(!date2.isEmpty()) {
				param.put("OPERT_END_DT", date2);
				model.addAttribute("OPERT_END_DT", date2);
				model.addAttribute("isSearchList", new Boolean(true));
			}
		}
		// 2.2 목록수
    	totalCount = cntcLogService.getTotalLogCount(fnIdx, param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = cntcLogService.getLogList(fnIdx, param);
		}
    	...
    	// 5. 추가경로
    	model.addAttribute("URL_LOGLIST", request.getAttribute("URL_LOGLIST"));
    	
		return getViewPath("/logList");
	}
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = cntcLogService.getTotalCount(fnIdx, param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = cntcLogService.getList(fnIdx, param);
		}
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 상세정보
		dt = cntcLogService.getView(fnIdx, param);
		if(dt == null) {
			// 해당글이 없는 경우
		...
		model.addAttribute("multiFileHashMap", cntcLogService.getMultiFileHashMap(fnIdx, keyIdx, settingInfo, items, itemOrder));	// multi file 목록
		model.addAttribute("multiDataHashMap", cntcLogService.getMultiHashMap(fnIdx, keyIdx, settingInfo, items, itemOrder));		// multi file 목록
	...
	public String input(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 상세정보
		dt = cntcLogService.getModify(fnIdx, param);
		if(dt == null) {
			// 해당글이 없는 경우
			model.addAttribute("message", MessageUtil.getAlert(rbsMessageSource.getMessage("message.no.contents")));
			return RbsProperties.getProperty("Globals.fail.path");
	...
	public String inputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 4. DB
    	int result = cntcLogService.update(uploadModulePath, fnIdx, keyIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 4.1 저장 성공
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
    	int result = cntcLogService.insert(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = cntcLogService.delete(fnIdx, parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String restore(@RequestParam(value="select") int[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = cntcLogService.restore(fnIdx, restoreIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String cdelete(@RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = cntcLogService.cdelete(uploadModulePath, fnIdx, deleteIdxs, settingInfo, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = cntcLogService.getDeleteCount(fnIdx, param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = cntcLogService.getDeleteList(fnIdx, param);
    	}
	...
	public ModelAndView fileUpload(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		Map<String, Object> fileInfo = cntcLogService.getFileUpload(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
	...
	public void fn_setAddCommonPath(ModuleAttrVO attrVO, HttpServletRequest request) {
		...
		String logListName = "logList.do";
	
		String logListNameUrl = logListName;
		
		if(!StringUtil.isEmpty(baseQueryString)) {
			
			logListNameUrl += baseQueryString;
			
		}
		
		request.setAttribute("URL_LOGLIST", logListNameUrl);
	}
	...
	public boolean isMngProcAuth(JSONObject settingInfo, int fnIdx, int keyIdx) {
		...
		if(!isMngAuth){
			...
			param.put("searchList", searchList);
			modiCnt = cntcLogService.getAuthCount(fnIdx, param);
