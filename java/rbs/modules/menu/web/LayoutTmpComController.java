...
public abstract class LayoutTmpComController extends LayoutComController{
	...
	public String list(String siteType, ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) {
		...
		// 1. 필수 parameter 검사
		/* 선택 모듈의 기능 목록 */
		settingInfo = JSONObjectUtil.getJSONObject(moduleSetting, "layoutTmp_setting_info");
		// 1. 페이지정보 setting
		...
		*/
		...
		layoutResourcePath += pathSeparator + siteType + RbsProperties.getProperty("Globals.layout.tmp.resource.path");
		...
		return getViewPath("/layoutTmpList");
	}
	...
	public String view(String siteType, String designName, ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 선택 모듈 상세 정보
		JSONObject moduleSetting = attrVO.getModuleSetting();
		JSONObject settingInfo = JSONObjectUtil.getJSONObject(moduleSetting, "layoutTmp_setting_info");
...
*/
    	...
    	// 기본경로
    	fn_setCommonPath(attrVO);
		return getViewPath("/layoutTmpView");
	}
