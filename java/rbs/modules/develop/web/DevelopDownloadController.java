...
public class DevelopDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = developService.getMultiFileView(targetTable, param);
			if(dt != null) {
				int result = developService.updateMultiFileDown(targetTable, keyIdx, fidx, itemId, targetTableIdx);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = developService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = developService.updateFileDown(targetTable, keyIdx, columnId);
			}
		}
