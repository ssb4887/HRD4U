...
public class StatisDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			//dt = statisService.getMultiFileView(fnIdx, param);
			if(dt != null) {
			//	int result = statisService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			//dt = statisService.getFileView(fnIdx, param);
			if(dt != null) {
			//	int result = statisService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
