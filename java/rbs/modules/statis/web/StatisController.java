package rbs.modules.statis.web;
...
import rbs.modules.statis.excel.ExcelExport;
import rbs.modules.statis.service.StatisService;
 ...
 */
@Controller
@ModuleMapping(moduleId="statis")
@RequestMapping({"/{siteId}/statis", "/{admSiteId}/menuContents/{usrSiteId}/statis"})
public class StatisController extends ModuleController {
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		String insttItemName = "instt";
		String instt = queryString.getString(searchKeyFlag + insttItemName);
		...
		// 년 검색
		String yearItemName = "year";
		String year = queryString.getString(searchKeyFlag + yearItemName);
		...
		// 월  검색
		String monthItemName = "month_multi";
		String month = queryString.getString(searchKeyFlag + monthItemName);
		if(StringUtil.isEmpty(month)) {
			monthItemName = "month";
			month = queryString.getString(searchKeyFlag + monthItemName);
		}
		if(!StringUtil.isEmpty(month)) {
			param.put("monthList", month);
			model.addAttribute("isSearchList", new Boolean(true));
			String[] monthList = StringUtil.stringToArray(month, ",");
			if(monthList != null) {
				int count = monthList.length;
				String monthSearch = null;
				
				for(int i = 0; i < count; i++) {
					monthSearch = monthList[i];
				}
				model.addAttribute("month", monthSearch);
			}
		}
		...
		// 2.2 목록수
    	totalCount = statisService.getTotalCount(param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = statisService.getList(param);
		}
	...
	public String totalList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		DataForm queryString = attrVO.getQueryString();
		boolean isAdmMode = attrVO.isAdmMode();
		...
		// 민간센터명 검색  
		String centerItemName = "center";
		String center = queryString.getString(searchKeyFlag + centerItemName);
		...
		// 기업유형 검색
		String typeItemName = "type";
		String type = queryString.getString(searchKeyFlag + typeItemName);
		...
		// 기업명 검색
		String corpItemName = "corp";
		String corp = queryString.getString(searchKeyFlag + corpItemName);
		...
		// 사업장관리번호 검색
		String bplNoItemName = "bplNo";
		String bplNo = queryString.getString(searchKeyFlag + bplNoItemName);
		if(!StringUtil.isEmpty(bplNo)) {
			bplNo = "%" + bplNo + "%";
			param.put("BPL", bplNo);
			model.addAttribute("isSearchList", new Boolean(true));
		}
		...
		if(instt != null) {
	    	...
	    	if(totalCount > 0) {
	    		...
	    		// 2.3 목록
	    		list = statisService.getCorpList(param);
			}
		}
		param.put("searchList", searchList);
    	...
    	// 5.추가경로
		model.addAttribute("CORP_LIST_URL", request.getAttribute("CORP_LIST_URL"));
	...
	public String total_excel(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 항목설정으로 검색조건 setting
		String listSearchId = "list_search";		// 검색설정 이름 구분해야 함
		JSONObject listSearch = JSONObjectUtil.getJSONObject(itemInfo, listSearchId);
		List<DTForm> itemInfoSearchList = ModuleUtil.getItemInfoSearchList(listSearch, queryString);
		if(itemInfoSearchList != null) {
			searchList.addAll(itemInfoSearchList);
	...
	@ModuleAuth(name="LST")
	@Auth(role = Role.R)
	@RequestMapping(value = "/corp_excel.do")
	public String corp_excel(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		DataForm queryString = attrVO.getQueryString();
		JSONObject itemInfo = attrVO.getItemInfo();
	...
	@ModuleAuth(name="LST")
	@Auth(role = Role.R)
	@RequestMapping(value = "/corp_excel2.do")
	public String corp_excel2(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model, HttpServletResponse response) throws Exception {
		DataForm queryString = attrVO.getQueryString();
		JSONObject itemInfo = attrVO.getItemInfo();
		...
		excelExport.write(response, "기업별_훈련참여정보");
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 상세정보
		dt = statisService.getView(param);
		if(dt == null) {
			// 해당글이 없는 경우
		...
		model.addAttribute("multiFileHashMap", statisService.getMultiFileHashMap(keyIdx, settingInfo, items, itemOrder));	// multi file 목록
		model.addAttribute("multiDataHashMap", statisService.getMultiHashMap(keyIdx, settingInfo, items, itemOrder));		// multi file 목록
	...
	public String input(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 상세정보
		dt = statisService.getModify(param);
		if(dt == null) {
			// 해당글이 없는 경우
			model.addAttribute("message", MessageUtil.getAlert(rbsMessageSource.getMessage("message.no.contents")));
			return RbsProperties.getProperty("Globals.fail.path");
	...
	public String inputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 4. DB
    	int result = statisService.update(uploadModulePath, keyIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 4.1 저장 성공
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
    	int result = statisService.insert(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = statisService.delete(parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String restore(@RequestParam(value="select") String[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = statisService.restore(restoreIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String cdelete(@RequestParam(value="select") String[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = statisService.cdelete(uploadModulePath, deleteIdxs, settingInfo, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = statisService.getDeleteCount(param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = statisService.getDeleteList(param);
    	}
	...
	public ModelAndView fileUpload(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		Map<String, Object> fileInfo = statisService.getFileUpload(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {	
		...
		String totalExcelUrl = "total_excel.do";									// 전체 통계 엑셀다운로드
		String corpExcelUrl = "corp_excel.do";										// 기업 통계 엑셀다운로드			
		String corpExcel2Url = "corp_excel2.do";									// 기업 통계 엑셀다운로드(자바)
		String corpListUrl = "corpList.do";										// 기업 통계 엑셀다운로드
		
		if(!StringUtil.isEmpty(baseQueryString)) {
			totalExcelUrl += baseQueryString;
			corpExcelUrl += baseQueryString;
			corpExcel2Url += baseQueryString;
			corpListUrl += baseQueryString;
		}
		request.setAttribute("TOTAL_EXCEL_URL", totalExcelUrl);
		request.setAttribute("CORP_EXCEL_URL", corpExcelUrl);
		request.setAttribute("CORP_EXCEL2_URL", corpExcel2Url);
		request.setAttribute("CORP_LIST_URL", corpListUrl);
	}
	...
	public boolean isMngProcAuth(JSONObject settingInfo, String keyIdx) {
		...
		if(!isMngAuth){
			...
			param.put("searchList", searchList);
			modiCnt = statisService.getAuthCount(param);
