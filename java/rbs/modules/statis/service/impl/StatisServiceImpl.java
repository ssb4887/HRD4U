package rbs.modules.statis.service.impl;
...
import rbs.modules.statis.mapper.StatisFileMapper;
import rbs.modules.statis.mapper.StatisMapper;
import rbs.modules.statis.mapper.StatisMultiMapper;
...
public class StatisServiceImpl extends EgovAbstractServiceImpl implements StatisService {
	...
	@Resource(name="statisMapper")
	private StatisMapper statisDAO;
	
	@Resource(name="statisFileMapper")
	private StatisFileMapper statisFileDAO;
	
	@Resource(name="statisMultiMapper")
	private StatisMultiMapper statisMultiDAO;
	...
	public int getTotalCount(Map<String, Object> param) {
    	return statisDAO.getTotalCount(param);
    }
	...
	public List<Object> getList(Map<String, Object> param) {
		...
		String preMonthList = StringUtil.getString(param.get("monthList"));
		
		String[] monthList = StringUtil.stringToArray(preMonthList, ",");
		param.put("MONTHLIST",monthList );
		
		return statisDAO.getList(param);
	}
	 ...
	 */
	@Override
	public List<Object> getCorpList(Map<String, Object> param) {
		String preMonthList = StringUtil.getString(param.get("monthList"));
		...
		return statisDAO.getCorpList(param);
	}
	...
	public DataMap getView(Map<String, Object> param) {
		DataMap viewDAO = statisDAO.getView(param);
		return viewDAO;
	}
	...
	public DataMap getFileView(Map<String, Object> param) {
		DataMap viewDAO = statisDAO.getFileView(param);
		return viewDAO;
	}
	...
	public DataMap getMultiFileView(Map<String, Object> param) {
		DataMap viewDAO = statisFileDAO.getFileView(param);
		return viewDAO;
	}
	...
	public int updateFileDown(String keyIdx, String fileColumnName) throws Exception{
		...
		return statisDAO.updateFileDown(param);
	...
	public int updateMultiFileDown(String keyIdx, int fleIdx, String itemId) throws Exception{
    	...
    	param.put("searchList", searchList);
		param.put("KEY_IDX", keyIdx);
		return statisFileDAO.updateFileDown(param);
	...
	public DataMap getModify(Map<String, Object> param) {
		DataMap viewDAO = statisDAO.getModify(param);
		return viewDAO;
	}
	...
	public int getAuthCount(Map<String, Object> param) {
    	return statisDAO.getAuthCount(param);
    }
	...
	public int insert(String uploadModulePath, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception  {
		...
		// 1. key 얻기
		String keyIdx = statisDAO.getNextId(param);
		...
		// 3. DB 저장
		// 3.1 기본 정보 테이블
		int result = statisDAO.insert(param);
		...
		if(result > 0) {
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = statisMultiDAO.insert(fileParam);
				}
			}
			...
			if(fileDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					int fleIdx = statisFileDAO.getNextId(fileParam);
					fileParam.put("FLE_IDX", fleIdx);
					result = statisFileDAO.insert(fileParam);
				}
			}
		}
	...
	public int update(String uploadModulePath, String keyIdx, String regiIp, ParamForm parameterMap, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
    	...
    	// 3. DB 저장
    	// 3.1 기본 정보 테이블
		int result = statisDAO.update(param);
		...
		if(result > 0){
			...
			result1 = statisMultiDAO.cdelete(multiDelParam);
			...
			if(multiDataList != null) {
				...
				for(int i = 0 ; i < multiDataSize ; i ++) {
					...
					result = statisMultiDAO.insert(multiParam);
				}
			}
			...
			if(fileModifyDataList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					result = statisFileDAO.update(fileParam);
				}
			}
			...
			if(fileDeleteSearchList != null) {
				...
				for(int i = 0 ; i < fileDataSize ; i ++) {
					...
					fileParam.put("KEY_IDX", keyIdx);
					// 6.1 삭제목록 select
					List<Object> deleteFileList2 = statisFileDAO.getList(fileParam);
					if(deleteFileList2 != null) deleteMultiFileList.addAll(deleteFileList2);
					// 6.2 DB 삭제
					result = statisFileDAO.cdelete(fileParam);
				}
	...
	public int getDeleteCount(Map<String, Object> param) {
    	return statisDAO.getDeleteCount(param);
    }
	...
	public List<Object> getDeleteList(Map<String, Object> param) {
		return statisDAO.getDeleteList(param);
	}
	...
	public int restore(String[] restoreIdxs, String regiIp, JSONObject settingInfo) throws Exception {
		...
		// 3. DB 저장
		return statisDAO.restore(param);
	}
	...
	public int cdelete(String uploadModulePath, String[] deleteIdxs, JSONObject settingInfo, JSONObject items, JSONArray itemOrder) throws Exception {
		...
		deleteMultiFileList = statisFileDAO.getList(param);
		...
		if(deleteFileColumnList != null) {
			...
			deleteFileList = statisDAO.getFileList(param);
		}
		
		// 3. delete
		int result = statisDAO.cdelete(param);
		if(result > 0) {
			// 4. 파일 삭제
			// 4.1 multi file 삭제
			String fileRealPath = RbsProperties.getProperty("Globals.upload.file.path") + File.separator + uploadModulePath;
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && objectType == 9) {
					...
					param.put("searchList", searchList);
					resultHashMap.put(itemId, statisFileDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
				...
				param.put("searchList", searchList);
				resultHashMap = statisFileDAO.getMapList(param);
			}
		}
		...
		if(!JSONObjectUtil.isEmpty(itemOrder) && !JSONObjectUtil.isEmpty(items)) {
			...
			for(int i = 0 ; i < searchOrderSize ; i ++) {
				...
				if(formatType == 0 && (objectType == 3 || objectType == 4 || objectType == 11)) {
			    	...
			    	searchList.add(new DTForm("A.ITEM_ID", itemId));
					param.put("searchList", searchList);
					resultHashMap.put(itemId, statisMultiDAO.getList(param));
					*/
				}
			}
			if(itemIdList.size() > 0) {
		    	...
		    	searchList.add(new DTForm("A.ITEM_ID", (String[])itemIdList.toArray(new String[itemIdList.size()])));
				param.put("searchList", searchList);
				resultHashMap = statisMultiDAO.getMapList(param);
			}
		}
