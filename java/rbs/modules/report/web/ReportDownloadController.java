...
public class ReportDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = reportService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				int result = reportService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = reportService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = reportService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
