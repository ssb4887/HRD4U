 ...
 */
@Controller
@ModuleMapping(moduleId="sprt", confModule="traing", confSModule="sprt")
@RequestMapping({"/{siteId}/sprt", "/{admSiteId}/menuContents/{usrSiteId}/sprt"})
public class TraingSprtController extends ModuleController {
	
	@Resource(name = "traingSprtService")
	private TraingSprtService traingSprtService;
	...
	public String detailList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = traingSprtService.getTotalCount(fnIdx, param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = traingSprtService.getDetailList(fnIdx, param);
		}
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
    	...
    	if(totalCount > 0) {
    		// 2.3 목록
    		dt = traingSprtService.getModify(fnIdx, param);
    		yearList = traingSprtService.getIndustCdYearList(fnIdx, param);
    		industCdList = traingSprtService.getIndustCdList(fnIdx, param);
		}
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		int count = traingSprtService.getTotalCount(fnIdx, param);
		if(count != 0) {
		...
		// 2. DB
    	int result = traingSprtService.insert(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String modify(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
    	...
    	if(totalCount > 0) {
    		// 2.3 목록
    		list = traingSprtService.getModify(fnIdx, param);
    		yearList = traingSprtService.getIndustCdYearList(fnIdx, param);
    		industCdList = traingSprtService.getIndustCdList(fnIdx, param);
	...
	public String modifyProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 4. DB
    	int result = traingSprtService.update(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 4.1 저장 성공
	...
	public String delete(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		int fnIdx = attrVO.getFnIdx();
		// 1. DB
		int result = traingSprtService.delete(fnIdx, parameterMap, request.getRemoteAddr(), settingInfo);
	...
	public boolean isMngProcAuth(JSONObject settingInfo, int fnIdx, int keyIdx) {
		...
		if(!isMngAuth){
			...
			param.put("searchList", searchList);
			modiCnt = traingSprtService.getAuthCount(fnIdx, param);
