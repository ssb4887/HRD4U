...
@RequestMapping({"/{siteId}/consulting", "/{admSiteId}/menuContents/{usrSiteId}/consulting"})
public class ConsultingDownloadController extends ModuleDownloadController{
	...
	@ModuleAuth(name={"DWN"})
	@RequestMapping(value = "/diaryDownload.do")
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		int keyIdx = StringUtil.getInt(request.getParameter(JSONObjectUtil.getString(settingInfo, "diary_name")));
		int fidx = StringUtil.getInt(request.getParameter("fidx"), 0);
		String itemId = request.getParameter("itId");
		if(fidx > 0 && StringUtil.isEmpty(itemId)) itemId = "photo";
		...
		searchList.add(new DTForm("A." + JSONObjectUtil.getString(settingInfo, "diary_column"), keyIdx));
		if(fidx > 0) {
			// multi file
			columnId = "FILE";
			...
			dt = consultingService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				int result = consultingService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = consultingService.getFileView(fnIdx, param);
			if(dt != null) {
				int result = consultingService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
