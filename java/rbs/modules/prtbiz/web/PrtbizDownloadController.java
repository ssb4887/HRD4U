...
public class PrtbizDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = prtbizService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				prtbizService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = prtbizService.getFileView(fnIdx, param);
			if(dt != null) {
				prtbizService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
