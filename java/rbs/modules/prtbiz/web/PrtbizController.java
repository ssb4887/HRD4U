package rbs.modules.prtbiz.web;
 ...
 */
@Controller
@ModuleMapping(moduleId="prtbiz")
@RequestMapping({"/{siteId}/prtbiz", "/{admSiteId}/menuContents/{usrSiteId}/prtbiz"})
public class PrtbizController extends ModuleController {
	
	@Resource(name = "prtbizService")
	private PrtbizService prtbizService;
	...
	public String list(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		String trainingItemName = "trainingName";
		String trainingName = queryString.getString(searchKeyFlag + trainingItemName);
		if(!StringUtil.isEmpty(trainingName)) {
			param.put("TP_NAME", "%" + trainingName + "%");
			model.addAttribute("isSearchList", new Boolean(true));
		}
		...
		// 2.2 목록수
    	totalCount = prtbizService.getTotalCount(fnIdx, param);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = prtbizService.getList(fnIdx, param);
		}
		...
		model.addAttribute("crtMenu", crtMenu);
	...
	public String view(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		DataMap dt = null;
		DataMap prtbizList = null;
		Map<String, Object> param = new HashMap<String, Object>();
		List<DTForm> searchList = new ArrayList<DTForm>();
		
		// 2.1 훈련과정 목록을 가져오기 위한 DB
		List<Object> trainingList = new ArrayList<Object>();
		List<Object> tpSubList = new ArrayList<Object>();
		Map<String, Object> param2 = new HashMap<String, Object>();
		List<DTForm> searchList2 = new ArrayList<DTForm>();
		List<Object> industList = new ArrayList<Object>();
		...
		// 상세정보
		dt = prtbizService.getView(fnIdx, param);
		if(dt == null) {
			// 해당글이 없는 경우
		...
		if(fnIdx == 1) {
			...
			int fnIdx2 = 2;
			searchList2.add(new DTForm("A.PRTBIZ_IDX", keyIdx));
			
			param2.put("searchList", searchList2);
			
			// 2.2 목록수
	    	totalCount = prtbizService.getTotalCount(fnIdx2, param);
			paginationInfo.setTotalRecordCount(totalCount);
	    	...
	    	if(totalCount > 0) {
	    		...
	    		// 2.3 목록
	    		trainingList = prtbizService.getList(fnIdx2, param);
			}

		} else {
			// 훈련과정 등록 시 참여가능사업 목록 가져오기
			tpSubList = prtbizService.getTpSubList(fnIdx, param);
			fnIdx = 1;
			prtbizList = prtbizService.getPrtbizList(fnIdx);
			industList = prtbizService.getIndustList(fnIdx);
		}
		...
		model.addAttribute("multiFileHashMap", prtbizService.getMultiFileHashMap(fnIdx, keyIdx, settingInfo, items, itemOrder));	// multi file 목록
		...
		model.addAttribute("prtbizList", prtbizList);											// 참여가능사업 목록
		model.addAttribute("paginationInfo", paginationInfo);										// 페이징정보
    	model.addAttribute("list", trainingList);	
		model.addAttribute("contextPath", request.getContextPath());
		model.addAttribute("crtSiteId", request.getAttribute("crtSiteId"));
		model.addAttribute("crtMenu", crtMenu);
		model.addAttribute("industList", industList);											// 업종코드 목록
		model.addAttribute("tpSubList", tpSubList);												// 훈련과정 목록
	...
	public String input(@RequestParam(value="mode") String mode, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1.3  작성자명 setting
		DataMap nameDt = new DataMap();
		
		String memberName = loginVO.getMemberNameOrg();
		nameDt.put("NAME", memberName);
		...
		List<DTForm> searchList = new ArrayList<DTForm>();
		List<Object> industList = new ArrayList<Object>();
		List<Object> industMaster = new ArrayList<Object>();
		List<Object> tpSubList = new ArrayList<Object>();
		...
		// 새로 추가되는 권한 체크
		/*Boolean isMngAuth = isMngProcAuth(siteId, menuIdx, memberIdx, userTypeIdx, menuType);
		if(!isMngAuth) {
			// 수정권한 없는 경우
			model.addAttribute("message", MessageUtil.getAlert(rbsMessageSource.getMessage("message.no.auth")));
			return RbsProperties.getProperty("Globals.fail.path");
		...
		// 2.2 상세정보
		dt = prtbizService.getModify(fnIdx, param);
		...
		// 훈련과정 등록 시 참여가능사업 목록, 업종코드 목록 가져오기
		if(fnIdx == 2) {
			fnIdx = 1;
			prtbizList = prtbizService.getPrtbizList(fnIdx);
			industList = prtbizService.getIndustList(fnIdx);
			industMaster = prtbizService.getIndustMasterCode(fnIdx);
		}
		...
		model.addAttribute("nameDt", nameDt);
		model.addAttribute("multiDataHashMap", prtbizService.getMultiHashMap(fnIdx, keyIdx, settingInfo, items, itemOrder));		// multi file 목록
		model.addAttribute("multiFileHashMap", prtbizService.getMultiFileHashMap(fnIdx, keyIdx, settingInfo, items, itemOrder));	// multi file 목록
		model.addAttribute("optnHashMap", CodeHelper.getItemOptnHashMap(items, itemOrder));		// 항목코드
		model.addAttribute("submitType", submitType);											// 페이지구분
		model.addAttribute("prtbizList", prtbizList);											// 참여가능사업 목록
		model.addAttribute("industList", industList);											// 업종코드 목록
		model.addAttribute("industMaster", industMaster);										// 업종코드 대분류 목록
		model.addAttribute("keyColumnName", JSONObjectUtil.getString(settingInfo, "idx_name"));
		model.addAttribute("keyIdx", keyIdx);
	...
	public String input(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 훈련과정 등록 시 참여가능사업 목록 & 업종 목록 가져오기
		if(fnIdx == 2) {
			fnIdx = 1;
			prtbizList = prtbizService.getPrtbizList(fnIdx);
			industList = prtbizService.getIndustList(fnIdx);
		...
		// 참여가능사업 상세보기에서 훈련과정 등록을 클릭할 때 해당 참여가능사업 idx setting
		int prtbizIdx = StringUtil.getInt(request.getParameter("prtbizIdx"));
    	...
    	model.addAttribute("prtbizList", prtbizList);
	 ...
	 * @return
	 * @throws Exception
	 * 추후에 적용업종을 코드로 관리하는 걸로 변경될 때 추가할 내용 > 파라미터에 @RequestParam(value="applyInduty", defaultValue="[]") String[] industIdxs 추가, String[] industIdx = null 삭제
	 */
	@Auth(role = Role.U)
	@ModuleAuth(name={"WRT"})
	...
	public String inputProc(@RequestParam(value="mode") String mode, @ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 4. DB
    	int result = prtbizService.update(uploadModulePath, fnIdx, keyIdx, request.getRemoteAddr(), parameterMap, attrVO);
    	if(result > 0) {
    		// 4.1 저장 성공
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		int result = prtbizService.insert(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
	...
	public String delete(@ParamMap ParamForm parameterMap, @RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = prtbizService.delete(fnIdx, parameterMap, deleteIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String delete22(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		boolean isAjax = attrVO.isAjax();
		String ajaxPName = attrVO.getAjaxPName();
		JSONObject settingInfo = attrVO.getSettingInfo();
		int fnIdx = attrVO.getFnIdx();
	...
	public String restore(@RequestParam(value="select") int[] restoreIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 1. DB
		int result = prtbizService.restore(fnIdx, restoreIdxs, request.getRemoteAddr(), settingInfo);
	...
	public String cdelete(@RequestParam(value="select") int[] deleteIdxs, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		JSONArray itemOrder = JSONObjectUtil.getJSONArray(itemInfo, submitType + "_order");
		// 1.2 삭제처리
		int result = prtbizService.cdelete(uploadModulePath, fnIdx, deleteIdxs, settingInfo, items, itemOrder);
	...
	public String deleteList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.2 목록수
    	totalCount = prtbizService.getDeleteCount(fnIdx, param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		list = prtbizService.getDeleteList(fnIdx, param);
    	}
	...
	public ModelAndView fileUpload(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
		Map<String, Object> fileInfo = prtbizService.getFileUpload(uploadModulePath, fnIdx, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
	...
	/**
	 * 참여가능사업 상세보기 페이지에서 훈련과정 목록 가져오기
	 * @param attrVO
	 * @param request
	 * @param model
	 * @return
	 ...
	 */
	@Auth(role = Role.R)
	@RequestMapping(method=RequestMethod.GET, value = "/tpList.do")
	public ModelAndView tpList(@ModuleAttr ModuleAttrVO attrVO, @RequestBody String json, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		int fnIdx = 2;
		JSONObject settingInfo = attrVO.getSettingInfo();
		boolean isAdmMode = attrVO.isAdmMode();
		JSONObject crtMenu = attrVO.getCrtMenu();
		...
		// 참여가능사업 정보 가져오기
		int keyIdx = StringUtil.getInt(request.getParameter("prtbizIdx"));
		...
		searchList.add(new DTForm("A.PRTBIZ_IDX", keyIdx));
    	...
    	if(totalCount > 0) {
    		...
    		// 2.3 목록
    		tpList = prtbizService.getList(fnIdx, param);
		}
		...
		mav.addObject("pageInfo", paginationInfo);
	...
	public void fn_setCommonPath(ModuleAttrVO attrVO) {	
		...
		String tpListUrl = "tpList.do";				// 훈련과정 목록
		if(!StringUtil.isEmpty(baseQueryString)) {
			tpListUrl += baseQueryString;
		}
		request.setAttribute("URL_TPLIST", tpListUrl);
	}
	...
	/*public boolean isMngProcAuth(JSONObject settingInfo, int fnIdx, int keyIdx) {
		...
		if(!isMngAuth){
			...
			param.put("searchList", searchList);
			modiCnt = prtbizService.getAuthCount(fnIdx, param);
