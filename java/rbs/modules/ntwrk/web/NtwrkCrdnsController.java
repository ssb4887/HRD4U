...
/**
 * 지역네트워크 > 유관기관 관리
 * (ntwrkCmptinst과 유사. 같은테이블 공유, 추후 설계변경가능성있음)
 * @author minjung
 *
 */
@Controller
@ModuleMapping(moduleId="crdns", confModule="ntwrk")
@RequestMapping({"/{siteId}/crdns", "/{admSiteId}/menuContents/{usrSiteId}/crdns"})
public class NtwrkCrdnsController extends ModuleController {
	...
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(NtwrkCrdnsController.class);
	...
	/**
	 * 유관기관 삭제하기(완전삭제X,ISDELETE=1로 업데이트)
	 * @param request
	 * @param values
	 * @return
	 * @throws Exception
	...
	public ModelAndView uploadExcel(@RequestParam("excelfile") MultipartFile file, HttpServletRequest request) throws IOException {
		ModelAndView mav = new ModelAndView();
		mav.setView(jsonView);
		
		// 엑셀 파일 외 업로드 불가 (FIX:16827)
		String fileRealName = file.getOriginalFilename();
		if(!fileRealName.endsWith(".xlsx")) {
		...
		if(workbook != null) {
			...
			while(rowIterator.hasNext()) {
				...
				map.put("cmptinstType", "CRDNS"); // 협의체
				paramList.add(map);
			}
		...
		mav.addObject("msg", result + "건의 유관기관이 등록되었습니다.");
		return mav;
	}
	...
	public String downloadExcel(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model, HttpServletResponse response) throws Exception {
		...
		// 본부는 모두 조회가능, 지부지사는 자신의 지부지사만 조회가능
		if(loginVO.getUsertypeIdx() < 40) {
	...
	public ModelAndView insert(@RequestParam Map<String, Object> formdata, HttpServletRequest request) {
		...
		formdata.put("cmptinstType", "CRDNS");
