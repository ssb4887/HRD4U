 ...
 */
@Controller
@ModuleMapping(moduleId="instt")
@RequestMapping({"/{siteId}/instt", "/{admSiteId}/menuContents/{usrSiteId}/instt", "/{admSiteId}/moduleFn/instt"})
public class InsttController extends ModuleController {
	...
	public String inputProc(@ParamMap ParamForm parameterMap, @ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		// 2. DB
    	int result = insttService.insert(uploadModulePath, request.getRemoteAddr(), parameterMap, settingInfo, items, itemOrder);
    	if(result > 0) {
    		// 2.1 저장 성공
        	// 기본경로
        	fn_setCommonPath(attrVO);
