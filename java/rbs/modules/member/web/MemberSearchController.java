...
@RequestMapping({"/{admSiteId}/menuContents/{usrSiteId}/member", "/{admSiteId}/moduleFn/member", "/{siteId}/member"})
public class MemberSearchController extends ModuleSearchController {
	 ...
	 * @throws Exception
	 */
	@ModuleAuth(name="LST", accessModule={"authManage"})
	@RequestMapping(value = "/usrSearchList.do")
	public String usrSearchList(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2.1 검색조건
		// 항목설정으로 검색조건 setting
		String listSearchId = "sublist_search";		// 검색설정
		JSONObject listSearch = JSONObjectUtil.getJSONObject(itemInfo, listSearchId);
		List<DTForm> itemInfoSearchList = ModuleUtil.getItemInfoSearchList(listSearch, queryString);
		if(itemInfoSearchList != null) {
			searchList.addAll(itemInfoSearchList);
		...
		// 2.2 목록수
    	totalCount = memberService.getDoctorCount(param);
		paginationInfo.setTotalRecordCount(totalCount);
    	...
    	// 기본경로
    	fn_setUsrSearchPath(attrVO);
		return getViewPath("/usrSearchList");
	}
	...
	public void fn_setUsrSearchPath(ModuleAttrVO attrVO) {
		...
		String listName = "usrSearchList.do";
		//String[] baseParams = StringUtil.addStringToArray(this.baseParams, "itemId");													// 기본 parameter
		PathUtil.fn_setListPath(queryString, baseParams, searchParams, pageName, listName);
	}
}
