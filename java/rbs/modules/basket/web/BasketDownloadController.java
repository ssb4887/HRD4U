...
public class BasketDownloadController extends ModuleDownloadController{
	...
	public ModelAndView download(@ModuleAttr ModuleAttrVO attrVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(fidx > 0) {
			...
			dt = basketService.getMultiFileView(fnIdx, param);
			if(dt != null) {
				basketService.updateMultiFileDown(fnIdx, keyIdx, fidx, itemId);
			}
		} else {
			...
			param.put("searchList", searchList);
			dt = basketService.getFileView(fnIdx, param);
			if(dt != null) {
				basketService.updateFileDown(fnIdx, keyIdx, columnId);
			}
		}
